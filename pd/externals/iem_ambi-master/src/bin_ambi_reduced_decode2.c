...
static void bin_ambi_reduced_decode2_calc_pinv(t_bin_ambi_reduced_decode2 *x)
{
	...
	if(x->x_beg_fade_out_hrir == 0)
	{
		if (!(a = (t_garray *)pd_findbyclass(x->x_s_fade_out_hrir, garray_class)))
			...
			pd_error(x, "%s: bad template for bin_ambi_reduced_decode2", x->x_s_fade_out_hrir->s_name);
...
static void bin_ambi_reduced_decode2_check_HRIR_arrays(t_bin_ambi_reduced_decode2 *x, t_float findex)
{
	...
	if (!(a = (t_garray *)pd_findbyclass(hrir, garray_class)))
		...
		pd_error(x, "%s: bad template for bin_ambi_reduced_decode2", hrir->s_name);
	else
	{
		if(npoints < fftsize)
		{
...
static void bin_ambi_reduced_decode2_check_HRTF_arrays(t_bin_ambi_reduced_decode2 *x, t_float findex)
{
	...
	if (!(a = (t_garray *)pd_findbyclass(hrtf_re, garray_class)))
		...
		pd_error(x, "%s: bad template for bin_ambi_reduced_decode2", hrtf_re->s_name);
		...
		pd_error(x, "%s: bad template for bin_ambi_reduced_decode2", hrtf_im->s_name);
	else if (npoints < fftsize)
