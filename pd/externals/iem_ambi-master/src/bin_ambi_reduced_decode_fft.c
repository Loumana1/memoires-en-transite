...
static void bin_ambi_reduced_decode_fft_calc_pinv(t_bin_ambi_reduced_decode_fft *x)
{
	...
	if(x->x_beg_fade_out_hrir == 0)
	{
		if (!(a = (t_garray *)pd_findbyclass(x->x_s_fade_out_hrir, garray_class)))
			...
			pd_error(x, "%s: bad template for bin_ambi_reduced_decode_fft", x->x_s_fade_out_hrir->s_name);
...
static void bin_ambi_reduced_decode_fft_check_HRIR_arrays(t_bin_ambi_reduced_decode_fft *x, t_float findex)
{
	...
	if (!(a = (t_garray *)pd_findbyclass(hrir, garray_class)))
		...
		pd_error(x, "%s: bad template for bin_ambi_reduced_decode_fft", hrir->s_name);
	else
	{
		if(npoints < fftsize)
		{
...
static void bin_ambi_reduced_decode_fft_check_HRTF_arrays(t_bin_ambi_reduced_decode_fft *x, t_float findex)
{
	...
	if (!(a = (t_garray *)pd_findbyclass(hrtf_re, garray_class)))
		...
		pd_error(x, "%s: bad template for bin_ambi_reduced_decode_fft", hrtf_re->s_name);
		...
		pd_error(x, "%s: bad template for bin_ambi_reduced_decode_fft", hrtf_im->s_name);
	else if (npoints < fftsize)
