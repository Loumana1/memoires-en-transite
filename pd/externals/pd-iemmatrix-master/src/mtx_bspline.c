...
static void mtx_bspline_float(t_mtx_spline *X, t_float f)
{
  ...
  for (j = 0; j < dim; j++) {
    ...
    t_float       ret =
        (t_float)(t * y[j][i + 1] + (1 - t) * y[j][i] +
                  uji * uji * (p[j][i + 1] * t3 + p[j][i] * t3i) / 6.0);
    SETFLOAT(result + j, ret);
  }
  outlet_list(X->x_outlet, 0, dim, result);
}
static void  mtx_bspline_free(t_mtx_spline *x) { mtx_bspline_resize(x, 0, 0); }
...
{
  mtx_bspline_class =
      class_new(gensym("mtx_bspline"), (t_newmethod)mtx_bspline_new,
                (t_method)mtx_bspline_free, sizeof(t_mtx_spline), 0, A_NULL);
  /*  class_addmethod(mtx_bspline_class, (t_method)mtx_bspline_matrix, gensym("list"), A_GIMME, 0); */
  class_addmethod(mtx_bspline_class, (t_method)mtx_bspline_matrix2, gensym(""),
                  A_GIMME, 0);
  class_addfloat(mtx_bspline_class, mtx_bspline_float);
}

void iemtx_bspline_setup(void) { mtx_bspline_setup(); }
