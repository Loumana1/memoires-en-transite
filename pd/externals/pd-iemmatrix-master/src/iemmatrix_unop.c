...
static void mtx_unop_matrix(t_matrix_unop *x, t_symbol *s, int argc,
                            t_atom *argv)
{
...
static void mtx_unop_list(t_matrix_unop *x, t_symbol *s, int argc, t_atom *argv)
{
  int     n = argc;
  t_atom *m;
  (void)s; /* unused */
...
static void mtx_unop_free(t_matrix_unop *x) { matrix_free(&x->m); }
                          ...
                          iemmatrix_unopfun_t *fun, ...)
{
  ...
  cls = class_new(s, (t_newmethod)mtx_unop_new, (t_method)mtx_unop_free,
                  sizeof(t_matrix_unop), 0, A_GIMME, A_NULL);

  class_addbang(cls, (t_method)mtx_unop_bang);
  class_addlist(cls, (t_method)mtx_unop_list);
  class_addmethod(cls, (t_method)mtx_unop_matrix, gensym("matrix"), A_GIMME,
                  A_NULL);
