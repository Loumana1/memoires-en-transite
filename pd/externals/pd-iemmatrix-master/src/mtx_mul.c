...
static void mtx_mul_matrix(t_mtx_binmtx *x, t_symbol *s, int argc, t_atom *argv)
{
  ...
  for (r = 0; r < row; r++)
    for (c = 0; c < col2; c++) {
      ...
      for (n = 0; n < col; n++) {
        sum += (t_matrixfloat)atom_getfloat(ap1 + col * r + n) *
               atom_getfloat(ap2 + col2 * n + c);
      }
...
static t_float binop(t_float f1, t_float f2) { return f1 * f2; }

void mtx_mul_setup(void)
{
  mtx_mul_class =
      class_new(gensym("mtx_mul"), (t_newmethod)mtx_mul_new,
                (t_method)mtx_binmtx_free, sizeof(t_mtx_binmtx), 0, A_GIMME, 0);
  ...
  class_addmethod(mtx_mul_class, (t_method)mtx_mul_matrix, gensym("matrix"),
                  A_GIMME, 0);
  class_addmethod(mtx_mul_class, (t_method)mtx_bin_matrix2, gensym(""), A_GIMME,
                  0);
  class_addfloat(mtx_mul_class, mtx_mul_float);
  class_addbang(mtx_mul_class, mtx_binmtx_bang);

  mtx_mulscalar_class =
      class_new(gensym("mtx_mul"), 0, (t_method)mtx_binscalar_free,
...
void iemtx_mul_setup(void) { mtx_mul_setup(); }
