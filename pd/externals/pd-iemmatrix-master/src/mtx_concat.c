...
static void mTXSetConcatMode(MTXconcat *mtx_concat_obj, t_symbol *c_mode)
{
  ...
  case 'c':
  case 'C':
  case ':': /* "column" */ mtx_concat_obj->concat_mode = 1; break;
  case 'r':
  case 'R': /* "row" */ mtx_concat_obj->concat_mode = 0; break;
  default:
...
static void *newMTXConcat(t_symbol *s, int argc, t_atom *argv)
{
  ...
  mtx_concat_obj->outl = outlet_new(&mtx_concat_obj->x_obj, gensym("matrix"));
...
static void mTXConcatMatrix2(MTXconcat *mtx_concat_obj, t_symbol *s, int argc,
                             t_atom *argv)
{
  (void)s;
                                        ...
                                        t_matrix *mtx_out)
{
  ...
  if (mtx1->row != mtx2->row) {
    pd_error(mtx_concat_obj,
             "[mtx_concat]: row-mode: matrices must have same number of rows!");
    return;
  }
  adjustsize(mtx_concat_obj, mtx_out, mtx1->row, mcols);
  ptr_out = mtx_out->atombuffer + 2;
  for (cnt = mtx1->row; cnt--;
       ptr_in1 += mtx1->col, ptr_in2 += mtx2->col, ptr_out += mtx_out->col) {
                                        ...
                                        t_matrix *mtx_out)
{
  ...
  if (mtx1->col != mtx2->col) {
    pd_error(
        mtx_concat_obj,
        "[mtx_concat]: col-mode: matrices must have same number of columns!");
    return;
  }
  adjustsize(mtx_concat_obj, mtx_out, mrows, mtx1->col);
  ptr_out = mtx_out->atombuffer + 2;
  for (cnt = mtx1->row; cnt--; ptr_in1 += mtx1->col, ptr_out += mtx_out->col) {
    memcpy(ptr_out, ptr_in1, mtx1->col * sizeof(t_atom));
  }
  for (cnt = mtx2->row; cnt--; ptr_in2 += mtx2->col, ptr_out += mtx_out->col) {
    memcpy(ptr_out, ptr_in2, mtx2->col * sizeof(t_atom));
  }

  mTXConcatBang(mtx_concat_obj);
}
static void mTXConcatMatrix(MTXconcat *mtx_concat_obj, t_symbol *s, int argc,
                            t_atom *argv)
{
  ...
  /* size check */
  if (iemmatrix_check(mtx_concat_obj, s, argc, argv, 0))
    return;
...
{
  mtx_concat_class = class_new(gensym("mtx_concat"), (t_newmethod)newMTXConcat,
                               (t_method)deleteMTXConcat, sizeof(MTXconcat),
                               CLASS_DEFAULT, A_GIMME, 0);
  class_addbang(mtx_concat_class, (t_method)mTXConcatBang);
  class_addmethod(mtx_concat_class, (t_method)mTXConcatMatrix, gensym("matrix"),
                  A_GIMME, 0);
  class_addmethod(mtx_concat_class, (t_method)mTXConcatMatrix2, gensym(""),
                  A_GIMME, 0);
  class_addmethod(mtx_concat_class, (t_method)mTXSetConcatMode, gensym("mode"),
                  A_DEFSYMBOL, 0);

  row_sym = gensym("row");
}

void iemtx_concat_setup(void) { mtx_concat_setup(); }
