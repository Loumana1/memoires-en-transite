...
void mtx_roll_setup(void)
{
  mtx_roll_class = class_new(gensym("mtx_roll"), (t_newmethod)mtx_roll_new,
                             (t_method)matrixobj_free, sizeof(t_mtx_binscalar),
                             0, A_DEFFLOAT, 0);
  class_addbang(mtx_roll_class, matrixobj_bang);
  class_addmethod(mtx_roll_class, (t_method)mtx_roll_matrix, gensym("matrix"),
                  A_GIMME, 0);
}
void iemtx_roll_setup(void) { mtx_roll_setup(); }
