...
static void mtx_prod_matrix(t_matrixobj *x, t_symbol *s, int argc, t_atom *argv)
{
  ...
  if (row * col > argc - 2) {
    pd_error(
        x, "[mtx_prod]: sparse matrices not yet supported : use \"mtx_check\"");
  } else {
...
{
  mtx_prod_class =
      class_new(gensym("mtx_prod"), (t_newmethod)mtx_prod_new,
                (t_method)matrixobj_free, sizeof(t_matrixobj), 0, 0);
  class_addlist(mtx_prod_class, mtx_prod_list);
  class_addmethod(mtx_prod_class, (t_method)mtx_prod_matrix, gensym("matrix"),
                  A_GIMME, 0);
}
void iemtx_prod_setup(void) { mtx_prod_setup(); }
