...
void matrix_element(t_matrixobj *x, t_symbol *s, int argc, t_atom *argv)
{
  ...
  case 1:
    ...
    outlet_float(x->x_obj.ob_outlet,
                 atom_getfloat(m->atombuffer + 2 + c + r * col));
    break;
  case 2:
...
static void matrix_list(t_matrixobj *x, t_symbol *s, int argc, t_atom *argv)
{
  ...
  if (argc < row * col) {
    pd_error(x,
             "[matrix]: sparse matrices not yet supported : use [mtx_check]!");
    return;
  }
...
static void *matrix_new(t_symbol *s, int argc, t_atom *argv)
{
  ...
  case 1:
    ...
    break;
  default: row = atom_getfloat(argv++); col = atom_getfloat(argv++);
  }
...
{
  matrix_class =
      class_new(gensym("matrix"), (t_newmethod)matrix_new,
  ...
  class_addmethod(matrix_class, (t_method)matrix_size, gensym("size"), A_GIMME,
                  0);
  class_addmethod(matrix_class, (t_method)matrixobj_eye, gensym("eye"), A_GIMME,
                  0);
  ...
  class_addmethod(matrix_class, (t_method)matrixobj_zeros, gensym("zeros"),
                  A_GIMME, 0);
  class_addmethod(matrix_class, (t_method)matrixobj_egg, gensym("egg"), A_GIMME,
                  0);
  ...
  class_addmethod(matrix_class, (t_method)matrix_row, gensym("row"), A_GIMME,
                  0);
  class_addmethod(matrix_class, (t_method)matrix_col, gensym("column"), A_GIMME,
                  0);
  class_addmethod(matrix_class, (t_method)matrix_col, gensym("col"), A_GIMME,
                  0);
  class_addmethod(matrix_class, (t_method)matrix_element, gensym("element"),
                  A_GIMME, 0);

  /* the file functions */
  class_addmethod(matrix_class, (t_method)matrix_write, gensym("write"),
                  A_SYMBOL, 0);
  class_addmethod(matrix_class, (t_method)matrix_read, gensym("read"), A_SYMBOL,
                  0);
...
void mtx_matrix_setup(void) { matrix_setup(); }

void iematrix_setup(void) { matrix_setup(); }
