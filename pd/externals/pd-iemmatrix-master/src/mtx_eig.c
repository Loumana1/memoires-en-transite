...
typedef int (*t_eigen_nonsymm)(gsl_matrix *, gsl_vector_complex *,
                               gsl_eigen_nonsymm_workspace *);
typedef int (*t_eigen_nonsymmv)(gsl_matrix *, gsl_vector_complex *,
                                gsl_matrix_complex *,
...
static void deleteMTXEig(MTXEig *x) { deleteMTXqlw(x); }
...
static void mTXEigMatrix(MTXEig *x, t_symbol *s, int argc, t_atom *argv)
{
  ...
  if (rows != columns) {
    pd_error(x,
             "[mtx_eig]: Eigendecomposition works for square matrices only!");
    return;
  }
  size      = rows;
  x->size   = size;
  ...
  switch (x->withevs) {
  case WITHOUTEVS: my_eigen_nonsymm(x->a, x->l, x->w); break;
  case WITHEVS:
...
{
  mtx_eig_class = class_new(gensym("mtx_eig"), (t_newmethod)newMTXEig,
                            (t_method)deleteMTXEig, sizeof(MTXEig),
  ...
  my_matrix_complex_alloc =
      iemmatrix_get_stub("gsl_matrix_complex_alloc", mtx_eig_class);
  my_matrix_complex_free =
      iemmatrix_get_stub("gsl_matrix_complex_free", mtx_eig_class);
  my_vector_complex_alloc =
      iemmatrix_get_stub("gsl_vector_complex_alloc", mtx_eig_class);
  my_vector_complex_free =
      iemmatrix_get_stub("gsl_vector_complex_free", mtx_eig_class);
  my_eigen_nonsymmv_alloc =
      iemmatrix_get_stub("gsl_eigen_nonsymmv_alloc", mtx_eig_class);
  my_eigen_nonsymmv_free =
      iemmatrix_get_stub("gsl_eigen_nonsymmv_free", mtx_eig_class);
  my_eigen_nonsymm_alloc =
      iemmatrix_get_stub("gsl_eigen_nonsymm_alloc", mtx_eig_class);
  my_eigen_nonsymm_free =
      iemmatrix_get_stub("gsl_eigen_nonsymm_free", mtx_eig_class);
...
#endif
  have_gsl = (my_matrix_alloc && my_matrix_free && my_vector_complex_alloc &&
              my_vector_complex_free && my_eigen_nonsymmv_alloc &&
              my_eigen_nonsymmv_free && my_matrix_complex_alloc &&
              my_matrix_complex_free && my_eigen_nonsymm_alloc &&
              my_eigen_nonsymm_free && my_eigen_nonsymm_free &&
              my_eigen_nonsymmv_free);
}

void iemtx_eig_setup(void) { mtx_eig_setup(); }
