                               ...
                               t_atom *argv)
{
  ...
  if (row == col) {
    ...
    if (row > col) {
      ...
      inverted    = mtx_doMultiply(col, transposed, row,
                                   mtx_doInvert(invertee, row, &err), row);
    }
  ...
  if (err) {
    ...
    pd_error(x, "mtx_inverse: couldn't really invert the matrix !!! %d error%c",
             err, (err - 1) ? 's' : 0);
  }
...
{
  mtx_inverse_class =
      class_new(gensym("mtx_inverse"), (t_newmethod)mtx_inverse_new,
...
void iemtx_inverse_setup(void) { mtx_inverse_setup(); }
