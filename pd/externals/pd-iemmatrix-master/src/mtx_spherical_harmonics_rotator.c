...
static void mat_mul_three(matrix *A, matrix *B, matrix *C, matrix *temp,
                          matrix *result)
{
  size_t i, j, k;
  for (i = 0; i < A->size1; ++i) {
...
static ivanic_s *ivanic_s_new(size_t N)
{
  ...
  matrix *matrices[] = {s->R,        s->Rz_alpha, s->Rz_beta,
                        s->Rz_gamma, s->ping,     s->pong};
  for (int i = 0; i < 6; ++i) {
    if (matrices[i] == NULL) {
      for (int j = 0; j < i; ++j)
...
static double P(int i, int l, int mu, int m_prime, matrix *R, int R_lm1_offset)
{
  ...
  if (m_prime == l) {
    return matrix_get(R, i, 3) * matrix_get(R, mu, twolm2) -
           matrix_get(R, i, 1 + 0) * matrix_get(R, mu, R_lm1_offset);
  }
  return matrix_get(R, i, 3) * matrix_get(R, mu, R_lm1_offset) +
         matrix_get(R, i, 1 + 0) * matrix_get(R, mu, twolm2);
}
...
static double V(int l, int m1, int m2, matrix *R, int R_lm1_offset)
{
  ...
  if (m1 > 0) {
    return P(1, l, m1 - 1, m2, R, R_lm1_offset) -
           P(-1, l, -m1 + 1, m2, R, R_lm1_offset);
  } else {
    return P(1, l, m1 + 1, m2, R, R_lm1_offset) +
           P(-1, l, -m1 - 1, m2, R, R_lm1_offset);
  }
...
static void uvw(double buffer[3], int l, int m1, int m2)
{
  ...
  buffer[1] =
      one_half * sqrt((double)v_num / denom) * (double)(1 - 2 * m1_is_zero);
  buffer[2] =
      -one_half * sqrt((double)w_num / denom) * (double)(1 - m1_is_zero);
}

void ivanic_ruedenberg_rotation_matrix(ivanic_s *s, double alpha, double beta,
                                       double gamma)
{
...
static t_mtx_spherical_harmonics_rotator *
mtx_spherical_harmonics_rotator_new(t_symbol *s, int argc, t_atom *argv)
{
  (void)s; // To suppress unused variable warning
  int                                N;
  t_mtx_spherical_harmonics_rotator *x =
      (t_mtx_spherical_harmonics_rotator *)pd_new(
          mtx_spherical_harmonics_rotator_class);
...
static void
mtx_spherical_harmonics_rotator_free(t_mtx_spherical_harmonics_rotator *x)
{
  if (x->list_rot) {
    free(x->list_rot);
  }
...
static void
mtx_spherical_harmonics_rotator_bang(t_mtx_spherical_harmonics_rotator *x)
{
  if (x->list_rot != 0) {
...
static void
mtx_spherical_harmonics_rotator_list(t_mtx_spherical_harmonics_rotator *x,
...
void mtx_spherical_harmonics_rotator_setup(void)
{
  mtx_spherical_harmonics_rotator_class = class_new(
      ...
      sizeof(t_mtx_spherical_harmonics_rotator), CLASS_DEFAULT, A_GIMME, 0);
  class_addbang(mtx_spherical_harmonics_rotator_class,
                (t_method)mtx_spherical_harmonics_rotator_bang);
  class_addlist(mtx_spherical_harmonics_rotator_class,
                (t_method)mtx_spherical_harmonics_rotator_list);
}
