...
static void mTXRepmatList(MTXrepmat *mtx_repmat_obj, t_symbol *s, int argc,
                          t_atom *argv)
{
  (void)s; /* unused */
  if (argc != 2) {
    pd_error(mtx_repmat_obj,
             "[mtx_repmat]: there have to be exactly 2 arguments");
    return;
  }
...
static void *newMTXRepmat(t_symbol *s, int argc, t_atom *argv)
{
  ...
  mtx_repmat_obj->list_outlet =
      outlet_new(&mtx_repmat_obj->x_obj, gensym("matrix"));
...
static void writeRepeatIntoMatrix(int repeat_rows, int repeat_cols, int rows,
...
static void mTXRepmatMatrix(MTXrepmat *mtx_repmat_obj, t_symbol *s, int argc,
                            t_atom *argv)
{
  ...
  /* size check */
  if (iemmatrix_check(mtx_repmat_obj, s, argc, argv, 0))
    return;
  ...
  if (size != mtx_repmat_obj->size) {
    if (list_out)
      list_out = (t_atom *)resizebytes(
          list_out, sizeof(t_atom) * (mtx_repmat_obj->size + 2),
  ...
  writeRepeatIntoMatrix(rep_rows, rep_cols, rows, columns, list_in,
                        list_out + 2);
...
{
  mtx_repmat_class = class_new(gensym("mtx_repmat"), (t_newmethod)newMTXRepmat,
                               (t_method)deleteMTXRepmat, sizeof(MTXrepmat),
                               CLASS_DEFAULT, A_GIMME, 0);
  class_addbang(mtx_repmat_class, (t_method)mTXRepmatBang);
  class_addmethod(mtx_repmat_class, (t_method)mTXRepmatMatrix, gensym("matrix"),
                  A_GIMME, 0);
  class_addmethod(mtx_repmat_class, (t_method)mTXRepmatList, gensym(""),
                  A_GIMME, 0);
}

void iemtx_repmat_setup(void) { mtx_repmat_setup(); }
