...
typedef struct _mtx_convolver_tilde {
  ...
  t_symbol *x_objname;
  setmultiout_f
      x_setmultiout; /* when doing multichannel, this is Pd>=0.54's signal_setmultiout(); otherwise NULL */
  ...
  t_float ***
      h; // 3D impulse response array h, h_num_ins x h_num_outs x h_len from array.h
...
static t_int *mtx_convolver_tilde_perform(t_int *w)
{
  ...
  unsigned int           available_inputs =
      (x->h_num_ins < x->ins) ? x->h_num_ins : x->ins;
  unsigned int available_outputs =
      (x->h_num_outs < x->outs) ? x->h_num_outs : x->outs;
  _debug_logpost(x, PD_DEBUG, "[%s] available ins=%d, outs=%d", objname,
                 available_inputs, available_outputs);
  if (x->conv) {
    _debug_logpost(x, PD_DEBUG, "[%s] processing with existing convolver",
                   objname);
    // Operation: copy input signals, convolve, copy output signals
    for (unsigned int i = 0; i < available_inputs;
         i++) { // copy from inout pd buffers
    ...
    for (unsigned int i = available_inputs; i < x->h_num_ins;
         i++) { // zeropad of unavailable convolver inputs
      t_float *in = x->conv_input_buffer[i];
      for (unsigned int n = 0; n < x->blocksize; n++) {
        in[n] = (t_float)0;
      }
    ...
    _debug_logpost(x, PD_DEBUG, "[%s] L=%d, P=%d, ins=%d, outs=%d", objname,
                   x->conv->blocksize, x->conv->num_partitions,
                   x->conv->num_inputs, x->conv->num_outputs);
    _debug_logpost(x, PD_DEBUG, "[%s] conv=%p, inbuf=%p, outbuf=%p", objname,
                   x->conv, x->conv_input_buffer, x->conv_output_buffer);
    IEMCONVOLVE(convProcess)(x->conv, x->conv_input_buffer,
                             x->conv_output_buffer);
    ...
    _debug_logpost(
        x, PD_DEBUG,
        "[%s] NOP with convolver inexistent, zeroing %d outs with blocksize %d",
        objname, x->outs, x->blocksize);
...
static void mtx_convolver_init(t_mtx_convolver_tilde *x)
{
  ...
  x->conv = 0;
  if ((x->blocksize > 0) && (x->h_num_ins > 0) && (x->h_num_outs > 0) &&
      (x->h_len > 0)) {
    unsigned int num_partitions = ceildiv(x->h_len, x->blocksize);
    x->conv = IEMCONVOLVE(initConvolution)(x->blocksize, num_partitions,
                                           x->blocksize, x->h_num_ins,
                                           x->h_num_outs, x->coherent_xfade);
  }
}
...
static int mtx_convolver_resize(t_mtx_convolver_tilde *x)
{
  ...
  if (x->conv) {
    unsigned int num_partitions = ceildiv(x->h_len, x->blocksize);
    if ((x->conv->blocksize == x->blocksize) &&
        (x->conv->num_partitions == num_partitions) &&
        (x->conv->num_inputs == x->h_num_ins) &&
        (x->conv->num_outputs == x->h_num_outs)) {
      _debug_logpost(x, PD_DEBUG,
                     "[%s] keeping convolver with ins=%d, outs=%d, "
                     "partitions=%d, blocksize=%d",
                     objname, x->conv->num_inputs, x->conv->num_outputs,
                     x->conv->num_partitions, x->conv->blocksize);
      ...
      if ((x->blocksize) && (x->h_num_ins) && (x->h_num_outs)) {
        x->conv_input_buffer =
            IEMCONVOLVE(new2DArray)(x->h_num_ins, x->blocksize);
        x->conv_output_buffer =
            IEMCONVOLVE(new2DArray)(x->h_num_outs, x->blocksize);
      }
    }
  } // new convolver:
  if ((x->blocksize) && (x->h_num_ins) && (x->h_num_outs) && (x->h_len)) {
    ...
    if (x->conv) {
      logpost(x, PD_DEBUG,
              "[%s] re-instantiated convolver with ins=%d, outs=%d, "
              "partitions=%d, blocksize=%d",
              objname, x->conv->num_inputs, x->conv->num_outputs,
              x->conv->num_partitions, x->conv->blocksize);
    }
...
static void mtx_convolver_tilde_dsp(t_mtx_convolver_tilde *x, t_signal **sp)
{
  ...
  if (x->x_setmultiout) {
    ...
    _debug_logpost(x, PD_DEBUG, "[%s] non-multichannel mode, in=%d, out=%d",
                   objname, ins, outs);
  }
#endif
  if (!x->inout_buffers) {
    x->inout_buffers =
        (t_float **)malloc(sizeof(t_float *) * (x->ins + x->outs));
  }
  x->blocksize = sp[0]->s_n > 0 ? sp[0]->s_n : 0;
  _debug_logpost(x, PD_DEBUG, "[%s] outs=%u, ins=%u, blocksize=%u", objname,
                 outs, ins, x->blocksize);
  int resized;
  ...
  if ((x->set_ir_at_dsp_start) || (resized)) {
    if (x->conv) {
      _debug_logpost(
          x, PD_DEBUG,
          "[%s] convolver has ins=%d, outs=%d, partitions=%d, blocksize=%d",
          objname, x->conv->num_inputs, x->conv->num_outputs,
          x->conv->num_partitions, x->conv->blocksize);
      IEMCONVOLVE(setImpulseResponseZeroPad)(
          x->conv, x->h, x->h_len,
          resized); // init with no xfade when conv was resized
                               ...
                               unsigned int tests)
{
  ...
  if (!tests)
    tests = IEMMATRIX_CHECK_CRIPPLED | IEMMATRIX_CHECK_DIMENSIONS |
            IEMMATRIX_CHECK_SPARSE;
  if ((tests & IEMMATRIX_CHECK_CRIPPLED) && argc < 3) {
    pd_error(x, "[%s] crippled array3", objname);
    return 1;
  }
  if ((tests & IEMMATRIX_CHECK_DIMENSIONS) &&
      ((inputs < 1) || (outputs < 1) || (length < 1))) {
    pd_error(x, "[%s] invalid dimensions %dx%dx%d", objname, inputs, outputs,
             length);
    return 1;
  }
  if ((tests & IEMMATRIX_CHECK_SPARSE) &&
      (inputs * outputs * length > argc - 3)) {
...
static void mtx_convolver_tilde_array3(t_mtx_convolver_tilde *x, t_symbol *s,
                                       int argc, t_atom *argv)
{
  ...
  if (argc < 3) {
    logpost(x, PD_ERROR,
            "[%s] %s message must have at least 3 arguments: num_inputs, "
            "num_outputs, ir_len",
            objname, s->s_name);
    return;
  }
  if (mtx_convolver_check(x, argc, argv, 0))
    return;
  ...
  argv += 3;
  _debug_logpost(x, PD_DEBUG,
                 "[%s] inputs=%d, outputs=%d, ir_len=%d, x->blocksize=%d",
                 objname, h_num_ins, h_num_outs, h_len, x->blocksize);

  if ((h_num_ins != x->h_num_ins) || (h_num_outs != x->h_num_outs) ||
      (h_len != x->h_len)) {
    _debug_logpost(x, PD_DEBUG,
                   "[%s] input array3: re-sizing/setting x->h_num_ins=%u, "
                   "x->h_num_outs=%u, x->h_len=%u",
                   objname, h_num_ins, h_num_outs, h_len);
    if (x->h)
      IEMCONVOLVE(free3DArray)(x->h, x->h_num_outs, x->h_num_ins);
  ...
  if (x->blocksize) { // if blocksize is already known
    int resized = mtx_convolver_resize(
        x); // check if new convolver needs to be instantiated
    if (x->conv) {
      if (canvas_dspstate) { // if convolver exists+dsp is running update IRs
        _debug_logpost(x, PD_DEBUG,
                       "[%s] attempting IR update with dsp on, ins=%d, "
                       "outs=%d, partitions=%d, blocksize=%d",
                       objname, x->conv->num_inputs, x->conv->num_outputs,
                       x->conv->num_partitions, x->conv->blocksize);
        IEMCONVOLVE(setImpulseResponseZeroPad)(
            x->conv, x->h, x->h_len,
            resized); // init with no xfade if conv was resized, with xfade otherwise
        if (resized_outs) {
          _debug_logpost(x, PD_DEBUG,
                         "[%s] convolver changed, calling dsp startup",
                         objname);
          canvas_update_dsp();
        }
      } else {
        ...
        _debug_logpost(x, PD_DEBUG,
                       "[%s] ir update postponed to dsp start, dsp is off",
                       objname);
      }
    } else {
      x->set_ir_at_dsp_start = 1;
      _debug_logpost(x, PD_DEBUG,
                     "[%s] ir update postponed to dsp start, convolver missing",
                     objname);
    }
  } else {
    x->set_ir_at_dsp_start = 1;
    _debug_logpost(x, PD_DEBUG,
                   "[%s] ir update postponed to dsp start, blocksize missing",
                   objname);
  }
}

static void mtx_convolver_tilde_matrix(t_mtx_convolver_tilde *x, t_symbol *s,
                                       int argc, t_atom *argv)
{
  t_atom *ap;
...
static void mtx_convolver_tilde_read(t_mtx_convolver_tilde *x,
                                     t_symbol              *filename)
{
  const char *objname = x->x_objname->s_name;
...
{
  t_symbol     *sym_signal = gensym("signal");
  setmultiout_f setmultiout =
      (CLASS_MULTICHANNEL) ? iemmatrix_getpdfun("signal_setmultiout") : 0;
  ...
  if (argc > 0 && argv[0].a_type == A_SYMBOL &&
    ...
    if (setmultiout)
      ...
      pd_error(x,
               "[%s] multichannel requested, but iemmatrix is running in "
               "Pd-%d.%d-%d, which doesn't support it",
  ...
  x->x_setmultiout =
      (mtx_convolver_tilde_class == selected_class) ? 0 : setmultiout;
  if (!x->x_setmultiout) {
    if (argc < 1) {
      x->ins = x->outs = 1;
    } else if (argc < 2) {
    ...
    for (unsigned int i = 0; i < x->ins; i++) {
      inlet_new(&x->x_obj, &x->x_obj.ob_pd, sym_signal, sym_signal);
    }
    for (unsigned int i = 0; i < x->outs; i++) {
  ...
  for (int i = 0; i < argc;
       i++) { // use first symbol after argv[0] as init file name
    if (argv[i].a_type == A_SYMBOL) {
      ...
      if (strcmp(matrix_file->s_name, "pow") != 0) {
        ...
        const char *objname = x->x_objname->s_name;
        logpost(x, PD_DEBUG,
                "[%s] activating power-complementary output crossfade",
                objname);
        x->coherent_xfade = 0;
      }
    }
...
{
  class_addmethod(c, (t_method)mtx_convolver_tilde_dsp, gensym("dsp"), A_CANT,
                  0);
  class_addmethod(c, (t_method)mtx_convolver_tilde_array3, gensym("array3"),
                  A_GIMME, 0);
  class_addmethod(c, (t_method)mtx_convolver_tilde_array3, gensym("#array3"),
                  A_GIMME, 0);
  class_addmethod(c, (t_method)mtx_convolver_tilde_read, gensym("read"),
                  A_SYMBOL, 0);
  class_addmethod(c, (t_method)mtx_convolver_tilde_matrix, gensym("matrix"),
                  A_GIMME, 0);
}
...
void iemtx_convolver__setup(void) { mtx_convolver_tilde_setup(); }
