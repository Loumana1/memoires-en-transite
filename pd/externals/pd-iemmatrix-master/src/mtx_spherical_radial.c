...
static void mTXSphMatrix(MTXSph *x, t_symbol *s, int argc, t_atom *argv)
{
  ...
  if ((rows != 1) || (columns < 1)) {
    pd_error(x, "[mtx_spherical_radial]: 1*L matrix expected with kr and h "
...
void mtx_spherical_radial_setup(void)
{
  mtx_spherical_radial_class = class_new(
      gensym("mtx_spherical_radial"), (t_newmethod)newMTXSph,
      (t_method)deleteMTXSphdata, sizeof(MTXSph), CLASS_DEFAULT, A_GIMME, 0);
...
void iemtx_spherical_radial_setup(void) { mtx_spherical_radial_setup(); }
