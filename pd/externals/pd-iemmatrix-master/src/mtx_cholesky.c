                                ...
                                t_atom *argv)
{
  ...
  if (iemmatrix_check(x, s, argc, argv, 0))
    return;
  if (row != col) {
    pd_error(x, "[mtx_cholesky]: only symmetric and positive definite matrices "
  ...
  for (i = 0; i < col; i++) {
    ...
    if ((result = original[i * (col + 1)] - sum) < 0) {
      pd_error(x, "[mtx_cholesky]: only symmetric and positive definite "
                  "matrices can be cholesky-decomposed");
      return;
    }
...
{
  mtx_cholesky_class =
      class_new(gensym("mtx_cholesky"), (t_newmethod)mtx_cholesky_new,
...
void iemtx_cholesky_setup(void) { mtx_cholesky_setup(); }
