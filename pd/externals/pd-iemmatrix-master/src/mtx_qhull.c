...
static void mTXQhullMatrix(MTXQhull *xo, t_symbol *s, int argc, t_atom *argv)
{
  ...
  /* size check */
  if (iemmatrix_check(xo, s, argc, argv, 0))
    return;
  ...
  if ((x != 0) && (y != 0) && (z != 0) && (xo->zh != 0)) {
    ...
    *(xo->zh) = zhullInitPoints(x, y, z, rows);
    ...
    if (xo->list == 0) {
      ...
      xo->list =
          (t_atom *)realloc(xo->list, (xo->hull_size * 3 + 2) * sizeof(t_atom));
    }
    if (xo->list != 0) {
      ...
      for (i = 0; i < xo->hull_size; i++) {
        SETFLOAT(xo->list + 2 + 3 * i,
                 (float)getTriangleCorner(xo->zh, i, 0) + 1);
        SETFLOAT(xo->list + 3 + 3 * i,
                 (float)getTriangleCorner(xo->zh, i, 1) + 1);
        SETFLOAT(xo->list + 4 + 3 * i,
                 (float)getTriangleCorner(xo->zh, i, 2) + 1);
      }
      outlet_anything(xo->outl, gensym("matrix"), xo->size, xo->list);
      freeZhull(xo->zh);
      free(xo->zh);
      xo->zh = 0;
    } else {
...
{
  mtx_qhull_class =
      class_new(gensym("mtx_qhull"), (t_newmethod)newMTXQhull,
                (t_method)deleteMTXQhull, sizeof(MTXQhull), CLASS_DEFAULT, 0);
  class_addmethod(mtx_qhull_class, (t_method)mTXQhullMatrix, gensym("matrix"),
                  A_GIMME, 0);
  //    class_addfloat(mtx_qhull_class, (t_method) mTXQhullSetIterations);
}

void iemtx_qhull_setup(void) { mtx_qhull_setup(); }
