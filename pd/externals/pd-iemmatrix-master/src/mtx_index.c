...
static void *newMTXIndex(t_symbol *s, int argc, t_atom *argv)
{
  ...
  mtx_index_obj->list_outlet =
      outlet_new(&mtx_index_obj->x_obj, gensym("matrix"));
  inlet_new(&mtx_index_obj->x_obj, &mtx_index_obj->x_obj.ob_pd,
            gensym("matrix"), gensym(""));

  pd_error(mtx_index_obj,
           "[%s]: this object is likely to change! not really for use yet",
           s->s_name);
...
static void mTXIndexRightMatrix(MTXindex *mtx_index_obj, t_symbol *s, int argc,
                                t_atom *argv)
{
  ...
  /* size check */
  if (iemmatrix_check(mtx_index_obj, s, argc, argv, 0))
    return;
  ...
  if (size != mtx_index_obj->index_size) {
    if (!index_in) {
      ...
      index_in = (int *)resizebytes(
          index_in, sizeof(int) * (mtx_index_obj->index_size + 2),
          sizeof(int) * (size + 2));
    if (!list_out) {
      list_out = (t_atom *)getbytes(sizeof(t_atom) * (size + 2));
    } else
      list_out = (t_atom *)resizebytes(
          list_out, sizeof(t_atom) * (mtx_index_obj->index_size + 2),
...
static void mTXIndexMatrix(MTXindex *mtx_index_obj, t_symbol *s, int argc,
                           t_atom *argv)
{
  int     rows          = atom_getint(argv + 0);
  int     columns       = atom_getint(argv + 1);
  ...
  if ((!indx) || (mtx_index_obj->index_size == 0)) {
    pd_error(mtx_index_obj,
             "[mtx_index]: index with what? no right matrix defined");
    return;
  }
  /* main part */
  list_out += 2;
...
{
  mtx_index_class = class_new(gensym("mtx_index"), (t_newmethod)newMTXIndex,
                              (t_method)deleteMTXIndex, sizeof(MTXindex),
                              CLASS_DEFAULT, A_GIMME, 0);
  class_addbang(mtx_index_class, (t_method)mTXIndexBang);
  class_addmethod(mtx_index_class, (t_method)mTXIndexMatrix, gensym("matrix"),
                  A_GIMME, 0);
  class_addmethod(mtx_index_class, (t_method)mTXIndexRightMatrix, gensym(""),
                  A_GIMME, 0);
}

void iemtx_index_setup(void) { mtx_index_setup(); }
