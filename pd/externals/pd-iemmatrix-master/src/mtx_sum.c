...
static void mtx_sum_matrix(t_matrixobj *x, t_symbol *s, int argc, t_atom *argv)
{
  int     row, col;
  int     n;
  t_atom *ap = 0, *dummy = 0;
...
{
  mtx_sum_class =
      class_new(gensym("mtx_sum"), (t_newmethod)mtx_sum_new,
...
void iemtx_sum_setup(void) { mtx_sum_setup(); }
