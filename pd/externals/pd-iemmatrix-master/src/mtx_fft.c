...
static void mtxFFTMatrixCold(MtxFFT *x, t_symbol *s, int argc, t_atom *argv)
{
  (void)s; /* unused */
...
static void mtxFFTMatrixHot(MtxFFT *x, t_symbol *s, int argc, t_atom *argv)
{
  ...
  if (msg_size != x->msg_size) {
    pd_error(x,
             "[mtx_fft]: left matrix has other dimensions than right matrix");
    return;
  } else if (columns < 4) {
    pd_error(x, "[mtx_fft]: matrix must have at least 4 columns");
    return;
...
{
  mtx_fft_class =
      class_new(gensym("mtx_fft"), (t_newmethod)newMtxFFT,
                (t_method)deleteMtxFFT, sizeof(MtxFFT), CLASS_DEFAULT, 0);
  class_addbang(mtx_fft_class, (t_method)mtxFFTBang);
  class_addmethod(mtx_fft_class, (t_method)mtxFFTMatrixHot, gensym("matrix"),
                  A_GIMME, 0);
...
void iemtx_fft_setup(void) { mtx_fft_setup(); }
