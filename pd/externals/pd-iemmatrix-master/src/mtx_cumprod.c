...
static void *newMTXCumprod(t_symbol *s, int argc, t_atom *argv)
{
  ...
  mtx_cumprod_obj->list_outlet =
      outlet_new(&mtx_cumprod_obj->x_obj, gensym("matrix"));
...
static void mTXCumprodMatrix(MTXCumprod *mtx_cumprod_obj, t_symbol *s, int argc,
                             t_atom *argv)
{
  ...
  /* size check */
  if (iemmatrix_check(mtx_cumprod_obj, s, argc, argv, 0))
    return;
  ...
  if ((!x) || (!list_out) || (!y)) {
    ...
    x = (t_float *)resizebytes(x, sizeof(t_float) * (mtx_cumprod_obj->size),
                               sizeof(t_float) * (size));
    y = (t_float *)resizebytes(y, sizeof(t_float) * (mtx_cumprod_obj->size),
                               sizeof(t_float) * (size));
    list_out = (t_atom *)resizebytes(
        list_out, sizeof(t_atom) * (mtx_cumprod_obj->size + 2),
...
{
  mtx_cumprod_class =
      class_new(gensym("mtx_cumprod"), (t_newmethod)newMTXCumprod,
                (t_method)deleteMTXCumprod, sizeof(MTXCumprod), CLASS_DEFAULT,
                A_GIMME, 0);
...
void iemtx_cumprod_setup(void) { mtx_cumprod_setup(); }
