...
static void *newMTXBessel(t_symbol *s, int argc, t_atom *argv)
{
  ...
  default:
  case 'J':
  case 'j': x->list_h_re_out = outlet_new(&x->x_obj, gensym("matrix")); break;
  case 'H':
  case 'h': x->list_h_re_out = outlet_new(&x->x_obj, gensym("matrix"));
  /* coverity[unterminated_case]: H has both real&imag outlet */ /* fall through */
  case 'Y':
  case 'y': x->list_h_im_out = outlet_new(&x->x_obj, gensym("matrix"));
  }
...
{
  if (x->list_h_im != 0) {
    outlet_anything(x->list_h_im_out, gensym("matrix"),
                    x->l * (x->nmax + 1) + 2, x->list_h_im);
  }
  if (x->list_h_re != 0) {
    outlet_anything(x->list_h_re_out, gensym("matrix"),
                    x->l * (x->nmax + 1) + 2, x->list_h_re);
  }
}

static void mTXBesselMatrix(MTXBessel *x, t_symbol *s, int argc, t_atom *argv)
{
  size_t columns;
  size_t n, m;
  (void)s; /* unused */
  ...
  if (my_jn && my_yn) {
    ...
    pd_error(x, "[mtx_bessel]: implementation requires math.h that implements "
                "jn and yn Bessel functions or gsl_sf_bessel.h");
  }
}

void mtx_bessel_setup(void)
{
  mtx_bessel_class = class_new(gensym("mtx_bessel"), (t_newmethod)newMTXBessel,
                               (t_method)deleteMTXBesseldata, sizeof(MTXBessel),
                               CLASS_DEFAULT, A_GIMME, 0);
  class_addbang(mtx_bessel_class, (t_method)mTXBesselBang);
  class_addmethod(mtx_bessel_class, (t_method)mTXBesselMatrix, gensym("matrix"),
                  A_GIMME, 0);
...
void iemtx_bessel_setup(void) { mtx_bessel_setup(); }
