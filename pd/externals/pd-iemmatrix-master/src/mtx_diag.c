...
static void mtx_diag_matrix(t_matrixobj *x, t_symbol *s, int argc, t_atom *argv)
{
...
static void *mtx_diag_new(t_symbol *s, int argc, t_atom *argv)
{
  ...
  argv += argc - 1;
  while (argc--) {
    SETFLOAT(x->m.atombuffer + 2 + argc * (1 + x->m.col),
             atom_getfloat(argv--));
  }
...
{
  mtx_diag_class =
      class_new(gensym("mtx_diag"), (t_newmethod)mtx_diag_new,
  ...
  class_addmethod(mtx_diag_class, (t_method)mtx_diag_matrix, gensym("matrix"),
                  A_GIMME, 0);
}
void iemtx_diag_setup(void) { mtx_diag_setup(); }
