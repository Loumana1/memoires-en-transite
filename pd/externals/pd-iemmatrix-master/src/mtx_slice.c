...
static void mTXSliceIndexList(MTXslice *mtx_slice_obj, t_symbol *s, int argc,
                              t_atom *argv)
{
  int       startcol;
  int       startrow;
  ...
  if (argc < 4) {
    pd_error(mtx_slice_obj, "[mtx_slice]: invalid index vector: "
  ...
  if (((startrow < 1) && (atom_getsymbol(&argv[0]) != endsym)) ||
    ...
    pd_error(mtx_slice_obj, "[mtx_slice]: row and column indices must be >0, "
                            "or misused \"end\" keyword");
    return;
  }

  if (((startrow > stoprow) && (atom_getsymbol(&argv[2]) != endsym)) ||
      ((startcol > stopcol) && (atom_getsymbol(&argv[3]) != endsym))) {
    pd_error(mtx_slice_obj, "[mtx_slice]: start_index<stop_index for rows and "
                            "columns, or misused \"end\" keyword");
    return;
  }
...
static void *newMTXSlice(t_symbol *s, int argc, t_atom *argv)
{
  ...
  mtx_slice_obj->list_outlet =
      outlet_new(&mtx_slice_obj->x_obj, gensym("matrix"));
  inlet_new(&mtx_slice_obj->x_obj, &mtx_slice_obj->x_obj.ob_pd, gensym("list"),
            gensym(""));
...
static void writeSliceIntoList(int slicerows, const int slicecols, int columns,
                               t_atom *x, t_atom *y)
{
...
static void mTXSliceMatrix(MTXslice *mtx_slice_obj, t_symbol *s, int argc,
                           t_atom *argv)
{
  ...
  /* size check */
  if (iemmatrix_check(mtx_slice_obj, s, argc, argv, 0))
    return;
  ...
  if ((stopcol > columns) || (stoprow > rows)) {
    pd_error(mtx_slice_obj,
             "[mtx_slice]: slice index exceeds matrix dimensions");
    return;
  }
  ...
  if (slicesize != mtx_slice_obj->slice_size) {
    if (!list_out) {
      ...
      list_out = (t_atom *)resizebytes(
          list_out, sizeof(t_atom) * (mtx_slice_obj->slice_size + 2),
  ...
  writeSliceIntoList(slicerows, slicecols, columns, list_in, list_out);
  list_out = mtx_slice_obj->list_out;
  SETSYMBOL(list_out, gensym("matrix"));
  SETFLOAT(list_out, slicerows);
  SETFLOAT(&list_out[1], slicecols);
  outlet_anything(mtx_slice_obj->list_outlet, gensym("matrix"), slicesize + 2,
                  list_out);
}

void mtx_slice_setup(void)
{
  mtx_slice_class = class_new(gensym("mtx_slice"), (t_newmethod)newMTXSlice,
                              (t_method)deleteMTXSlice, sizeof(MTXslice),
                              CLASS_DEFAULT, A_GIMME, 0);
  class_addbang(mtx_slice_class, (t_method)mTXSliceBang);
  class_addmethod(mtx_slice_class, (t_method)mTXSliceMatrix, gensym("matrix"),
                  A_GIMME, 0);
  class_addmethod(mtx_slice_class, (t_method)mTXSliceIndexList, gensym(""),
                  A_GIMME, 0);
}

void iemtx_slice_setup(void) { mtx_slice_setup(); }
