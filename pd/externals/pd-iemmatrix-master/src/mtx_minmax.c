...
static void *newMTXMinMax(t_symbol *s)
{
  ...
  mtx_minmax_obj->min_outlet =
      outlet_new(&mtx_minmax_obj->x_obj, gensym("matrix"));
  mtx_minmax_obj->max_outlet =
      outlet_new(&mtx_minmax_obj->x_obj, gensym("matrix"));

  if ((NULL != s) && (gensym("") != s) && (NULL != s->s_name)) {
...
static void minmaxListRows(int rows, int columns, t_atom *x, t_atom *ap_min,
                           t_atom *ap_max)
{
  int     count;
  t_float min, max;
...
static void mTXMinMaxMatrix(MTXminmax *mtx_minmax_obj, t_symbol *s, int argc,
                            t_atom *argv)
{
  ...
  /* size check */
  (void)s; /* unused */
  if (iemmatrix_check(mtx_minmax_obj, s, argc, argv, 0))
    return;
  ...
  if (size != mtx_minmax_obj->size) {
    if (!minlist_out) {
      ...
      minlist_out = (t_atom *)resizebytes(
          minlist_out, sizeof(t_atom) * (mtx_minmax_obj->size),
          sizeof(t_atom) * size);
    if (!maxlist_out) {
      maxlist_out = (t_atom *)getbytes(sizeof(t_atom) * size);
    } else
      maxlist_out = (t_atom *)resizebytes(
          maxlist_out, sizeof(t_atom) * (mtx_minmax_obj->size),
          sizeof(t_atom) * size);
  }
...
{
  mtx_minmax_class = class_new(gensym("mtx_minmax"), (t_newmethod)newMTXMinMax,
                               (t_method)deleteMTXMinMax, sizeof(MTXminmax),
                               CLASS_DEFAULT, A_DEFSYM, 0);

  class_addbang(mtx_minmax_class, (t_method)mTXMinMaxBang);
  class_addmethod(mtx_minmax_class, (t_method)mTXMinMaxMatrix, gensym("matrix"),
                  A_GIMME, 0);
  class_addmethod(mtx_minmax_class, (t_method)mTXSetMinMaxMode, gensym("mode"),
                  A_DEFSYMBOL, 0);
...
void iemtx_minmax_setup(void) { mtx_minmax_setup(); }
