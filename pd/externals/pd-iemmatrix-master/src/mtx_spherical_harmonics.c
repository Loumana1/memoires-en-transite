...
static void allocMTXShdata(MTXSh *x)
{
  ...
  x->list_sh = (t_atom *)calloc(x->l * (x->nmax + 1) * (x->nmax + 1) + 2,
                                sizeof(t_atom));
}
...
static void _mTXShMatrix(MTXSh *x, unsigned int rows, unsigned int columns,
                         t_atom *argv)
{
  unsigned int size = rows * columns;

  if ((rows != 2) || (columns < 1)) {
    pd_error(x, "[mtx_spherical_harmonics]: 2 X L matrix expected with phi and "
                "theta vector, but got more rows/no entries");
    return;
  }
...
static void mTXShMatrix(MTXSh *x, t_symbol *s, int argc, t_atom *argv)
{
...
static void mTXShList(MTXSh *x, t_symbol *s, int argc, t_atom *argv)
{
  (void)s;
  if (2 != argc) {
...
{
  if (x->list_ch != 0) {
    outlet_anything(x->list_ch_out, gensym("matrix"),
                    x->l * (2 * x->nmax + 1) + 2, x->list_ch);
  }
}
...
static void mTXChMatrix(MTXCh *x, t_symbol *s, int argc, t_atom *argv)
{
  ...
  if ((rows != 1) || (columns < 1)) {
    pd_error(x, "[mtx_circular_harmonics]: 1*L matrix expected with phi "
                "vector, but got more rows/no entries");
    return;
  }
...
void mtx_spherical_harmonics_setup(void)
{
  mtx_spherical_harmonics_class = class_new(
      gensym("mtx_spherical_harmonics"), (t_newmethod)newMTXSh,
      (t_method)deleteMTXShdata, sizeof(MTXSh), CLASS_DEFAULT, A_GIMME, 0);
...
void mtx_circular_harmonics_setup(void)
{
  mtx_circular_harmonics_class = class_new(
      gensym("mtx_circular_harmonics"), (t_newmethod)newMTXCh,
      (t_method)deleteMTXChdata, sizeof(MTXCh), CLASS_DEFAULT, A_GIMME, 0);
