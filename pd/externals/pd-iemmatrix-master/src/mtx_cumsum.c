...
static void *newMTXCumsum(t_symbol *s, int argc, t_atom *argv)
{
    ...
    if (argv[0].a_type == A_SYMBOL) {
      ...
      if (argc >= 2) {
        if (argv[1].a_type != A_SYMBOL) {
          ...
          pd_error(mtx_cumsum_obj,
                   "[%s]: 2nd arg ignored. supposed to be float", s->s_name);
        }
      }
    } else {
  ...
  mtx_cumsum_obj->list_outlet =
      outlet_new(&mtx_cumsum_obj->x_obj, gensym("matrix"));
...
static void mTXCumsumMatrix(MTXCumsum *mtx_cumsum_obj, t_symbol *s, int argc,
                            t_atom *argv)
{
  ...
  /* size check */
  if (iemmatrix_check(mtx_cumsum_obj, s, argc, argv, 0))
    return;
  ...
  if ((!x) || (!list_out) || (!y)) {
    ...
    x = (t_float *)resizebytes(x, sizeof(t_float) * (mtx_cumsum_obj->size),
                               sizeof(t_float) * (size));
    y = (t_float *)resizebytes(y, sizeof(t_float) * (mtx_cumsum_obj->size),
                               sizeof(t_float) * (size));
    list_out = (t_atom *)resizebytes(
        list_out, sizeof(t_atom) * (mtx_cumsum_obj->size + 2),
...
{
  mtx_cumsum_class = class_new(gensym("mtx_cumsum"), (t_newmethod)newMTXCumsum,
                               (t_method)deleteMTXCumsum, sizeof(MTXCumsum),
                               CLASS_DEFAULT, A_GIMME, 0);
  class_addbang(mtx_cumsum_class, (t_method)mTXCumsumBang);
  class_addmethod(mtx_cumsum_class, (t_method)mTXCumsumMatrix, gensym("matrix"),
                  A_GIMME, 0);
  class_addmethod(mtx_cumsum_class, (t_method)mTXSetCumsumMode, gensym("mode"),
                  A_DEFSYMBOL, 0);
...
void iemtx_cumsum_setup(void) { mtx_cumsum_setup(); }
