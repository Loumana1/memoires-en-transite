...
{
  mtx_check_class =
      class_new(gensym("mtx_check"), (t_newmethod)mtx_check_new,
                (t_method)matrixobj_free, sizeof(t_matrixobj), 0, 0);
  class_addbang(mtx_check_class, matrixobj_bang);
  class_addmethod(mtx_check_class, (t_method)mtx_check_matrix, gensym("matrix"),
                  A_GIMME, 0);
}

void iemtx_check_setup(void) { mtx_check_setup(); }
