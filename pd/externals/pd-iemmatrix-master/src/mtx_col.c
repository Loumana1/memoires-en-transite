...
static void mtx_col_matrix(t_matrixobj *x, t_symbol *s, int argc, t_atom *argv)
{
...
static void mtx_col_list(t_matrixobj *x, t_symbol *s, int argc, t_atom *argv)
{
  ...
  if (argc < x->m.row) {
    pd_error(x, "[mtx_col]: column length is too small for %dx%d-matrix",
             x->m.row, x->m.col);
    return;
  }
  if (x->current_col > x->m.col) {
...
{
  mtx_col_class =
      class_new(gensym("mtx_col"), (t_newmethod)mtx_col_new,
  ...
  class_addmethod(mtx_col_class, (t_method)mtx_col_matrix, gensym("matrix"),
                  A_GIMME, 0);
  class_addmethod(mtx_col_class, (t_method)mtx_col_float, gensym(""), A_FLOAT,
                  0);
}
