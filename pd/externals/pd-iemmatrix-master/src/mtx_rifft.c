...
static void mTXRifftMatrixCold(MTXRifft *x, t_symbol *s, int argc, t_atom *argv)
{
  ...
  if (columns_re < 3) {
    ...
    pd_error(x,
             "[mtx_rifft]: sparse matrix not yet supported: use [mtx_check]");
    return;
  } else if (columns < 4) {
  ...
  if ((x->rows != rows) || (columns != x->columns)) {
    ...
    t_complex *c_in = x->c_in =
        (t_complex *)realloc(x->c_in, sizeof(*x->c_in) * list_size);
    t_float *f_out = x->f_out =
        (t_float *)realloc(x->f_out, sizeof(*x->f_out) * size);
    ...
    for (int r = 0; r < x->rows; r++) {
      x->plan[r] = iemfft_plan_rifft_1d(columns, c_in + r * columns_re,
                                        f_out + r * columns, PREFER_DOUBLE);
    }

    x->list_re = list_re =
        (t_atom *)realloc(list_re, sizeof(*list_re) * (size + 2));
  }
...
static void mTXRifftMatrixHot(MTXRifft *x, t_symbol *s, int argc, t_atom *argv)
{
  int        rows       = atom_getint(argv++);
  int        columns_re = atom_getint(argv++);
  ...
  if ((rows != x->rows) || (columns != ((columns_re - 1) << 1))) {
    pd_error(x,
             "[mtx_rifft]: matrix dimensions do not match: expected %dx%d, got "
             "%dx%d[%d]",
             x->rows, x->columns, rows, ((columns_re - 1) << 1), columns_re);
    return;
  } else if (in_size < list_size) {
    pd_error(x,
             "[mtx_rifft]: sparse matrix not yet supported: use [mtx_check]");
...
{
  if (x->list_re)
    outlet_anything(x->list_re_out, gensym("matrix"), x->size + 2, x->list_re);
}
...
{
  mtx_rifft_class =
      class_new(gensym("mtx_rifft"), (t_newmethod)newMTXRifft,
                (t_method)deleteMTXRifft, sizeof(MTXRifft), CLASS_DEFAULT, 0);
  class_addbang(mtx_rifft_class, (t_method)mTXRifftBang);
  class_addmethod(mtx_rifft_class, (t_method)mTXRifftMatrixHot,
                  gensym("matrix"), A_GIMME, 0);
  class_addmethod(mtx_rifft_class, (t_method)mTXRifftMatrixCold, gensym(""),
                  A_GIMME, 0);

  iemfft_init(mtx_rifft_class);
}

void iemtx_rifft_setup(void) { mtx_rifft_setup(); }
