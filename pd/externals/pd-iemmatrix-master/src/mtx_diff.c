...
static void deleteMTXdiff(MTXdiff *mtx_diff_obj)
{
  if (mtx_diff_obj->list_out) {
    freebytes(mtx_diff_obj->list_out,
    ...
    if (argv[0].a_type == A_SYMBOL) {
      ...
      if (argc >= 2) {
        if (argv[1].a_type != A_SYMBOL) {
          ...
          pd_error(mtx_diff_obj,
                   "[mtx_diff]: 2nd arg ignored. supposed to be float");
        }
      }
    } else {
      ...
      if (argc >= 2) {
        if (argv[1].a_type == A_SYMBOL) {
          ...
          pd_error(mtx_diff_obj, "[mtx_diff]: 2nd arg ignored. supposed to be "
                                 "symbolic, e.g. \"row\", \"col\", \":\"");
        }
      }
    }
  }

  mtx_diff_obj->list_outlet =
      outlet_new(&mtx_diff_obj->x_obj, gensym("matrix"));
...
static void mTXdiffMatrix(MTXdiff *mtx_diff_obj, t_symbol *s, int argc,
                          t_atom *argv)
{
  ...
  /* size check */
  if (iemmatrix_check(mtx_diff_obj, s, argc, argv, 0))
    return;
  ...
  if ((!x) || (!list_out) || (!y)) {
    ...
    x        = (t_float *)resizebytes(x, sizeof(t_float) * (mtx_diff_obj->size),
                                      sizeof(t_float) * (size));
    y        = (t_float *)resizebytes(y, sizeof(t_float) * (mtx_diff_obj->size),
                                      sizeof(t_float) * (size));
...
{
  mtx_diff_class = class_new(gensym("mtx_diff"), (t_newmethod)newMTXdiff,
                             (t_method)deleteMTXdiff, sizeof(MTXdiff),
                             CLASS_DEFAULT, A_GIMME, 0);
  class_addbang(mtx_diff_class, (t_method)mTXdiffBang);
  class_addmethod(mtx_diff_class, (t_method)mTXdiffMatrix, gensym("matrix"),
                  A_GIMME, 0);
...
void iemtx_diff_setup(void) { mtx_diff_setup(); }
