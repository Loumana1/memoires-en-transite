...
void adjustsize(void *x, t_matrix *m, int desiredRow, int desiredCol)
{
  ...
  if (col * row != desiredRow * desiredCol) {
    ...
    m->atombuffer =
        (t_atom *)getbytes((desiredCol * desiredRow + 2) * sizeof(t_atom));
  }
...
{
  logpost(x, PD_NORMAL, "matrix@%p [%dx%d] :: %p", m, m ? m->row : 0,
          m ? m->col : 0, m ? (m->atombuffer + 2) : 0);
  if (!m)
    return;
...
t_matrixfloat *matrix2float(t_atom *ap)
{
  ...
  int            length = row * col;
  t_matrixfloat *buffer =
      (t_matrixfloat *)getbytes(sizeof(t_matrixfloat) * length);
...
void matrixobj_bang(t_matrixobj *x) { matrix_bang(&x->x_obj, &x->m); }
...
void matrixobj_free(t_matrixobj *x) { matrix_free(&x->m); }
/* set data */
...
void matrix_zeros(t_object *x, t_matrix *m, int argc, t_atom *argv)
{
  ...
  switch (argc) {
  case 0: /* zero out the actual matrix */ matrix_set(m, 0); break;
  case 1:
    row = atom_getfloat(argv);
...
void matrix_ones(t_object *x, t_matrix *m, int argc, t_atom *argv)
{
  ...
  switch (argc) {
  case 0: /* zero out the actual matrix */ matrix_set(m, 1); break;
  case 1:
    row = atom_getfloat(argv);
    adjustsize(x, m, row, row);
    matrix_set(m, 1);
...
int iemmatrix_check(void *object, t_symbol *s, int argc, t_atom *argv,
                    unsigned int tests)
{
...
void iemmatrix_floats2list_modulo(t_atom *dest, const t_float *src, size_t n,
                                  size_t m)
{
  t_atom *ptr = dest;
  int     count1, count2;
...
void iemmatrix_list2floats_modulo(t_float *dest, const t_atom *src, size_t n,
                                  size_t m)
{
...
struct _iemmatrix_map *iemmatrix_map_add(struct _iemmatrix_map *map,
                                         t_symbol *key, void *value)
{
...
void iemmatrix_map_free(struct _iemmatrix_map *map,
