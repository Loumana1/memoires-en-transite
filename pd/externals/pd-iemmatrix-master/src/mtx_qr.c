...
typedef int (*t_linalg_QR_unpack)(gsl_matrix *, gsl_vector *, gsl_matrix *,
                                  gsl_matrix *);
...
static void deleteMTXQr(MTXQr *x) { deleteMTXqr(x); }
...
static void mTXQrBang(MTXQr *x)
{
  if (x->list_q) {
    outlet_anything(x->list_r_out, gensym("matrix"), x->rows * x->columns + 2,
                    x->list_r);
    outlet_anything(x->list_q_out, gensym("matrix"), x->rows * x->rows + 2,
                    x->list_q);
  }
}

static void mTXQrMatrix(MTXQr *x, t_symbol *s, int argc, t_atom *argv)
{
...
{
  mtx_qr_class =
      class_new(gensym("mtx_qr"), (t_newmethod)newMTXQr, (t_method)deleteMTXQr,
                sizeof(MTXQr), CLASS_DEFAULT, A_GIMME, 0);
  ...
  my_vector_alloc =
      iemmatrix_get_stub("gsl_vector_complex_alloc", mtx_qr_class);
  my_vector_free = iemmatrix_get_stub("gsl_vector_complex_free", mtx_qr_class);
  my_linalg_QR_decomp =
      iemmatrix_get_stub("gsl_linalg_QR_decomp", mtx_qr_class);
  my_linalg_QR_unpack =
      iemmatrix_get_stub("gsl_linalg_QR_unpack", mtx_qr_class);
#endif
  have_gsl = (my_matrix_alloc && my_matrix_free && my_vector_alloc &&
              my_vector_free && my_linalg_QR_decomp && my_linalg_QR_unpack);
}

void iemtx_qr_setup(void) { mtx_qr_setup(); }
