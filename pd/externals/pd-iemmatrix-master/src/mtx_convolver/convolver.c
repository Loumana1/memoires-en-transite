...
/* crossfade functions */
/*-----------------------------------------------------------------------------------------------------------------------------*/
void IEMCONVOLVE(crossFade)(t_float *y, t_float *y_new, t_float *w_old,
                            t_float *w_new, unsigned int len)
{
...
void IEMCONVOLVE(convProcess)(conv_data *conv, t_float **in, t_float **out)
{
  for (unsigned int in_ch = 0; in_ch < conv->num_inputs; in_ch++) {
    IEMCONVOLVE(copyArray)(conv->x_old[in_ch], conv->xtemp,
                           conv->blocksize); // copy old signal block
    IEMCONVOLVE(copyArray)(in[in_ch], &conv->xtemp[conv->blocksize],
                           conv->blocksize); // append new signal block
    iemfft_execute(conv->fftplan_xtemp);     // perform 2L FFT to xftemp
    IEMCONVOLVE(copyComplexArray)(
        conv->xftemp, conv->xf[in_ch][conv->current_rb],
        conv->blocksize + 1); // write FFT into ring buffer
    IEMCONVOLVE(copyArray)(in[in_ch], conv->x_old[in_ch],
                           conv->blocksize); // store current signal block
  }
  for (unsigned int out_ch = 0; out_ch < conv->num_outputs; out_ch++) {
    IEMCONVOLVE(resetComplexArray)(
        conv->yftemp,
        conv->blocksize + 1); // zero output's partition accumulator
    for (unsigned int in_ch = 0; in_ch < conv->num_inputs;
         in_ch++) { // MIMO convolutions (main)
      for (unsigned int p = 0, px = conv->current_rb; p < conv->num_partitions;
           p++, px++) {
        IEMCONVOLVE(freq_mul_acc)(conv->xf[in_ch][px % conv->num_partitions],
                                  conv->hf[conv->current_cf][out_ch][in_ch][p],
                                  conv->yftemp, conv->blocksize + 1);
      }
    }
    iemfft_execute(
        conv->ifftplan_y); // perform iFFT of the main-IR output partition
    ...
    if (IEMCONVOLVE(wasCrossFadeRegistered(conv))) {
      ...
      IEMCONVOLVE(resetComplexArray)(
          conv->yftemp,
          conv->blocksize + 1); // zero output's partitions accumulator
      for (unsigned int in_ch = 0; in_ch < conv->num_inputs;
           in_ch++) { // MIMO convolutions (update)
        for (unsigned int p = 0, px = conv->current_rb;
             p < conv->num_partitions; p++) {
          IEMCONVOLVE(freq_mul_acc)(conv->xf[in_ch][px % conv->num_partitions],
                                    conv->hf[current_cf][out_ch][in_ch][p],
                                    conv->yftemp, conv->blocksize + 1);
          px++;
        }
      }
      iemfft_execute(
          conv->ifftplan_y_cf); // perform iFFT of the updated-IR output partition
      IEMCONVOLVE(crossFade)(
          conv->y + conv->blocksize, conv->y_cf + conv->blocksize, conv->w_old,
          conv->w_new, conv->blocksize); // xfade main->updated
    }
    /* IF IR WAS UPDATED: CROSSFADE TO NEW OUTPUT OF THE OUT CHANNEL COMPLETE */
    IEMCONVOLVE(copyArrayWithGain)(
        &conv->y[conv->blocksize], out[out_ch], conv->blocksize,
...
/*-----------------------------------------------------------------------------------------------------------------------------*/
conv_data *IEMCONVOLVE(initConvolution)(unsigned int blocksize,
                                        ...
                                        _Bool        coherent_xfade)
{
  ...
  conv->xtemp =
      IEMCONVOLVE(new1DArray)(conv->blocksize * 2); // input signal FFT buffer
  conv->htemp = IEMCONVOLVE(new1DArray)(conv->blocksize *
                                        2); // IR signal partition FFT buffer
  conv->y     = IEMCONVOLVE(new1DArray)(conv->blocksize *
                                        2); // output signal FFT buffer main/old
  conv->y_cf  = IEMCONVOLVE(new1DArray)(
      conv->blocksize * 2); // output signal FFT buffer new for crossfade
  conv->w_old =
      IEMCONVOLVE(new1DArray)(conv->blocksize); // fade-out window signal buffer
  conv->w_new =
      IEMCONVOLVE(new1DArray)(conv->blocksize); // fade-in window signal buffer
  conv->current_rb = 0; // partition ringbuffer index [0;P-1]
  conv->current_cf = 0; // crossfade-from index {0, 1}
  conv->xfade_length =
      (xfade_length < blocksize)
          ? xfade_length
          : blocksize; // crossfade length (<=L) for Hann window
  IEMCONVOLVE(resetArray)(conv->xtemp, conv->blocksize * 2);
  IEMCONVOLVE(resetArray)(conv->htemp, conv->blocksize * 2);
  IEMCONVOLVE(resetArray)(&conv->htemp[conv->blocksize],
                          conv->blocksize); // zero pad
  ...
  conv->x_old = IEMCONVOLVE(new2DArray)(
      conv->num_inputs, conv->blocksize); // old input signal buffer
  ...
  conv->xf = IEMCONVOLVE(new3DComplexArray)(
      conv->num_inputs, conv->num_partitions, conv->blocksize + 1);
  IEMCONVOLVE(reset3DComplexArray)(conv->xf, conv->num_inputs,
                                   conv->num_partitions, conv->blocksize + 1);
  // frequency-domain buffers holding all MIMO ir partitions
  conv->hf = IEMCONVOLVE(new5DComplexArray)(
      NUM_CF, conv->num_outputs, conv->num_inputs, conv->num_partitions,
      conv->blocksize + 1);
  IEMCONVOLVE(reset5DComplexArray)(conv->hf, NUM_CF, conv->num_outputs,
                                   conv->num_inputs, conv->num_partitions,
                                   conv->blocksize + 1);
  ...
  conv->fftplan_xtemp = iemfft_plan_rfft_1d(
      conv->blocksize * 2, conv->xtemp, conv->xftemp, DEFAULT | PRESERVE_INPUT);
  conv->fftplan_htemp = iemfft_plan_rfft_1d(
      conv->blocksize * 2, conv->htemp, conv->hftemp, DEFAULT | PRESERVE_INPUT);
  // single-channel IFFT plan for temporary main/old and new output signal
  conv->ifftplan_y    = iemfft_plan_rifft_1d(conv->blocksize * 2, conv->yftemp,
                                             conv->y, DEFAULT | PRESERVE_INPUT);
  conv->ifftplan_y_cf = iemfft_plan_rifft_1d(
      conv->blocksize * 2, conv->yftemp, conv->y_cf, DEFAULT | PRESERVE_INPUT);
  return conv;
}
...
void IEMCONVOLVE(freeConvolution)(conv_data *conv)
{
  ...
  IEMCONVOLVE(free3DComplexArray)(conv->xf, conv->num_inputs,
                                  conv->num_partitions);
  // frequency-domain array with all MIMO ir partitions
  IEMCONVOLVE(free5DComplexArray)(conv->hf, NUM_CF, conv->num_outputs,
                                  conv->num_inputs, conv->num_partitions);
...
/*-----------------------------------------------------------------------------------------------------------------------------*/
void IEMCONVOLVE(setImpulseResponseZeroPad)(conv_data *conv, t_float ***inh,
                                            ...
                                            _Bool        no_xfade_init)
{
  ...
  for (unsigned int out_ch = 0; out_ch < conv->num_outputs; out_ch++) {
    for (unsigned int in_ch = 0; in_ch < conv->num_inputs; in_ch++) {
      offset = 0;
      for (unsigned int partition = 0; partition < conv->num_partitions;
           partition++) {
        copy_length = (offset + conv->blocksize > num_samples)
                          ? num_samples - offset
                          : conv->blocksize;
        IEMCONVOLVE(copyArray)(&inh[out_ch][in_ch][offset], conv->htemp,
                               copy_length);
        IEMCONVOLVE(resetArray)(&conv->htemp[copy_length],
                                conv->blocksize - copy_length);
