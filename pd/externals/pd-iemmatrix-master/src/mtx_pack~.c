...
static int should_defer(mtx_pack_tilde *x)
{
  ...
  if (KERNEL_VERSION(major, minor, bugfix) >= KERNEL_VERSION(0, 56, 0))
    return 1;
  pd_error(x, "starting with Pd-0.56, [mtx_pack~] will stop sending messages "
              "in the DSP-tick!");
...
void deleteMtxPackTilde(mtx_pack_tilde *x)
{
  ...
  if (x->list_out) {
    freebytes(x->list_out,
              (x->num_channels * x->block_size + 2) * sizeof(*x->list_out));
  }
  if (x->clock)
    clock_free(x->clock);
}
...
static void mTxPackTildeDsp(mtx_pack_tilde *x, t_signal **sp)
{
  ...
  outlet_anything(x->info_outlet, gensym("channels"), 1, x->list_out + 0);
  outlet_anything(x->info_outlet, gensym("blocksize"), 1, x->list_out + 1);
  outlet_anything(x->info_outlet, gensym("dimen"), 2, x->list_out);
}
...
void mtx_pack_tilde_setup(void)
{
  ...
  mtx_pack_tilde_class =
      class_new(gensym("mtx_pack~"), (t_newmethod)newMtxPackTilde,
                (t_method)deleteMtxPackTilde, sizeof(mtx_pack_tilde), flags,
                A_DEFFLOAT, 0);
  class_addmethod(mtx_pack_tilde_class, (t_method)mTxPackTildeDsp,
                  gensym("dsp"), A_CANT, 0);

  can_multiout =
      (CLASS_MULTICHANNEL && 0 != iemmatrix_getpdfun("signal_setmultiout"));
}
