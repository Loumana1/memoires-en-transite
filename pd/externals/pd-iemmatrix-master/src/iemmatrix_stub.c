...
static module_t getmodule(const char *modulename, const char *path)
{
  ...
  if (!mod && path) {
    ...
    if (snprintf(fullmodulename, MAXPDSTRING, "%s/%s", path, modulename) <
        MAXPDSTRING)
...
static module_t getstubmodule(const char *basename, const char *path)
{
  ...
  snprintf(modulename, MAXPDSTRING - 1, "libiemmatrixStub_%s.%s", basename,
           extension);
...
#define DECLARE_STUB(fun) static void *iemmatrix_stub_##fun = 0
