...
static void mTXSetReverseMode(MTXreverse *mtx_reverse_obj, t_symbol *c_mode)
{
  ...
  case 'c':
  case 'C':
  case ':': /* "column" */ mtx_reverse_obj->reverse_mode = 1; break;
  case 'r':
  case 'R': /* "row" */ mtx_reverse_obj->reverse_mode = 0; break;
  case 'e':
  case 'E':
...
static void *newMTXreverse(t_symbol *s, int argc, t_atom *argv)
{
  ...
  mtx_reverse_obj->list_outlet =
      outlet_new(&mtx_reverse_obj->x_obj, gensym("matrix"));
...
static void mTXreverseMatrix(MTXreverse *mtx_reverse_obj, t_symbol *s, int argc,
                             t_atom *argv)
{
  ...
  if (!size) {
    ...
    pd_error(mtx_reverse_obj,
             "mtx_reverse: sparse matrix not yet supported: use \"mtx_check\"");
    return;
  }
  ...
  if (size != mtx_reverse_obj->size) {
    if (!list_out) {
      ...
      list_out = (t_atom *)resizebytes(
          list_out, sizeof(t_atom) * (mtx_reverse_obj->size + 2),
  ...
  case 1:
    ...
    break;
  default: reverseList(size, list_out);
  }
...
{
  mtx_reverse_class =
      class_new(gensym("mtx_reverse"), (t_newmethod)newMTXreverse,
                (t_method)deleteMTXreverse, sizeof(MTXreverse), CLASS_DEFAULT,
                A_GIMME, 0);
...
void iemtx_reverse_setup(void) { mtx_reverse_setup(); }
