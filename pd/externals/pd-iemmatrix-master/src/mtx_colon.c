...
static void *newMTXColon()
{
  ...
  mtx_colon_obj->list_outlet =
      outlet_new(&mtx_colon_obj->x_obj, gensym("matrix"));
...
static void mTXColonList(MTXColon *mtx_colon_obj, t_symbol *s, int argc,
                         t_atom *argv)
{
  int     size;
  t_float startval;
    ...
    if (size != mtx_colon_obj->size) {
      if (list_out)
        list_out = (t_atom *)resizebytes(
            list_out, sizeof(t_atom) * (mtx_colon_obj->size + 2),
...
static void mTXColonMtx(MTXColon *mtx_colon_obj, t_symbol *s, int argc,
                        t_atom *argv)
{
  ...
  t_atom *list_out = mtx_colon_obj->list_out;
  (void)s; /* unused */
  if (iemmatrix_check(mtx_colon_obj, s, argc, argv, 0))
    return;
...
{
  mtx_colon_class =
      class_new(gensym("mtx_colon"), (t_newmethod)newMTXColon,
                (t_method)deleteMTXColon, sizeof(MTXColon), CLASS_DEFAULT, 0);
...
void iemtx_colon_setup(void) { mtx_colon_setup(); }
