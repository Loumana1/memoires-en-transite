...
static void deleteMTXFill(MTXfill *mtx_fill_obj)
{
  if (mtx_fill_obj->list_out) {
    freebytes(mtx_fill_obj->list_out,
...
static void mTXFillIndexMatrix(MTXfill *mtx_fill_obj, t_symbol *s, int argc,
                               t_atom *argv)
{
...
static void *newMTXFill(t_symbol *s, int argc, t_atom *argv)
{
  ...
  mtx_fill_obj->fill_type     = DONT_FILL_JUST_PASS;
  pd_error(mtx_fill_obj, "[%s]: this object _might_ change in the future!",
           s->s_name);
  if (argc) {
    if (atom_getsymbol(argv) == gensym("matrix")) {
      ...
      pd_error(mtx_fill_obj,
               "%s: creation argument must be 'matrix <startrow> <startcol>' "
               "for submatrix filling or 'matrix rows columns [...]' for "
               "indexed filling with scalar/matrices",
               s->s_name);
    }
  }

  mtx_fill_obj->list_outlet =
      outlet_new(&mtx_fill_obj->x_obj, gensym("matrix"));
  inlet_new(&mtx_fill_obj->x_obj, &mtx_fill_obj->x_obj.ob_pd, gensym("matrix"),
            gensym("fill_mtx"));
  inlet_new(&mtx_fill_obj->x_obj, &mtx_fill_obj->x_obj.ob_pd, gensym("matrix"),
            gensym("index"));
...
static void mTXBigMatrix(MTXfill *mtx_fill_obj, t_symbol *s, int argc,
                         t_atom *argv)
{
  int     rows     = atom_getint(argv);
  ...
  /* size check */
  if (iemmatrix_check(mtx_fill_obj, s, argc, argv, 0))
    return;
  ...
  if (size != mtx_fill_obj->size) {
    if (!list_out) {
      ...
      list_out = (t_atom *)resizebytes(
          list_out, sizeof(t_atom) * (mtx_fill_obj->size + 2),
          sizeof(t_atom) * (size + 2));
  }
...
static void mTXFillScalar(MTXfill *mtx_fill_obj, t_float f)
{
  ...
  case FILL_SUBMATRIX:
    pd_error(mtx_fill_obj,
             "[mtx_fill]: scalar fill for submatrices not supported yet");
    return;
    break;
  ...
  case FILL_INDEXED_ELEMENTS:
    if (mtx_fill_obj->max_index > mtx_fill_obj->size) {
      pd_error(mtx_fill_obj,
               "[mtx_fill]: index matrix index exceeds matrix borders");
      return;
    }
    ...
    /* fall through */
  default: mTXFillBang(mtx_fill_obj);
  }
}

static void mTXFillMatrix(MTXfill *mtx_fill_obj, t_symbol *s, int argc,
                          t_atom *argv)
{
  ...
  case FILL_SUBMATRIX:
    if (list_size < fill_size) {
      pd_error(mtx_fill_obj,
               "[mtx_fill]: sparse matrix not yet supported: use [mtx_check]");
      return;
    }
    if ((stopcol > columns) || (stoprow > rows)) {
      pd_error(mtx_fill_obj,
               "[mtx_fill]: fill matrix index exceeds matrix borders");
      return;
    }
    break;
  case FILL_INDEXED_ELEMENTS:
    if (list_size < mtx_fill_obj->num_idcs_used) {
      pd_error(mtx_fill_obj,
               "[mtx_fill]: fill matrix smaller than indexing vector");
      return;
  ...
  case FILL_SUBMATRIX:
    list_out += columns * (mtx_fill_obj->fill_startrow - 1) +
                mtx_fill_obj->fill_startcol - 1;
    writeFillMatrixIntoList(fill_rows, fill_columns, columns, fill_mtx,
                            list_out + 2);
    break;
  case FILL_INDEXED_ELEMENTS:
...
{
  mtx_fill_class = class_new(gensym("mtx_fill"), (t_newmethod)newMTXFill,
                             (t_method)deleteMTXFill, sizeof(MTXfill),
                             CLASS_DEFAULT, A_GIMME, 0);
  class_addbang(mtx_fill_class, (t_method)mTXFillBang);
  class_addmethod(mtx_fill_class, (t_method)mTXFillMatrix, gensym("matrix"),
                  A_GIMME, 0);
  class_addmethod(mtx_fill_class, (t_method)mTXBigMatrix, gensym("fill_mtx"),
                  A_GIMME, 0);
  class_addmethod(mtx_fill_class, (t_method)mTXFillIndexMatrix, gensym("index"),
                  A_GIMME, 0);
  class_addfloat(mtx_fill_class, (t_method)mTXFillScalar);
}

void iemtx_fill_setup(void) { mtx_fill_setup(); }
