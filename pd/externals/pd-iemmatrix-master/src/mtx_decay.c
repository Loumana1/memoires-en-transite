...
static void *newMTXDecay(t_symbol *s, int argc, t_atom *argv)
{
  ...
  if (sym_count > 1) {
    pd_error(
        mtx_decay_obj,
        "[mtx_decay]: args after pos %d ignored. supposed to be non-symbolic",
        first_sym);
    argc = first_sym + 1;
  }
  ...
  mtx_decay_obj->list_outlet =
      outlet_new(&mtx_decay_obj->x_obj, gensym("matrix"));
...
static void mTXDecayMatrix(MTXDecay *mtx_decay_obj, t_symbol *s, int argc,
                           t_atom *argv)
{
  ...
  /* size check */
  if (iemmatrix_check(mtx_decay_obj, s, argc, argv, 0))
    return;
  if ((!x) || (!list_out) || (!y)) {
    if (!x) {
    ...
    x = (t_float *)resizebytes(x, sizeof(t_float) * (mtx_decay_obj->size),
                               sizeof(t_float) * (size));
    y = (t_float *)resizebytes(y, sizeof(t_float) * (mtx_decay_obj->size),
                               sizeof(t_float) * (size));
...
{
  mtx_decay_class = class_new(gensym("mtx_decay"), (t_newmethod)newMTXDecay,
                              (t_method)deleteMTXDecay, sizeof(MTXDecay),
                              CLASS_DEFAULT, A_GIMME, 0);
  class_addbang(mtx_decay_class, (t_method)mTXDecayBang);
  class_addmethod(mtx_decay_class, (t_method)mTXDecayMatrix, gensym("matrix"),
                  A_GIMME, 0);
  class_addmethod(mtx_decay_class, (t_method)mTXSetDecayParameter,
                  gensym("alpha"), A_DEFFLOAT, 0);
  class_addmethod(mtx_decay_class, (t_method)mTXSetDecayMode, gensym("mode"),
                  A_DEFSYMBOL, 0);
...
void iemtx_decay_setup(void) { mtx_decay_setup(); }
