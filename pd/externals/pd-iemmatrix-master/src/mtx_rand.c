...
static void mtx_rand_seed(t_matrixobj *x, t_float f) { x->current_row = f; }
...
static void mtx_rand_list(t_matrixobj *x, t_symbol *s, int argc, t_atom *argv)
{
  ...
  switch (argc) {
  case 0: return;
  case 1: row = col = atom_getfloat(argv); break;
  default: row = atom_getfloat(argv + 0); col = atom_getfloat(argv + 1);
  }
...
static void mtx_rand_matrix(t_matrixobj *x, t_symbol *s, int argc, t_atom *argv)
{
  (void)s;
...
{
  mtx_rand_class =
      class_new(gensym("mtx_rand"), (t_newmethod)mtx_rand_new,
                (t_method)matrixobj_free, sizeof(t_matrixobj), 0, A_GIMME, 0);
  class_addmethod(mtx_rand_class, (t_method)mtx_rand_matrix, gensym("matrix"),
                  A_GIMME, 0);
