...
static void *newMTXConv()
{
  ...
  mtx_conv_obj->list_outlet =
      outlet_new(&mtx_conv_obj->x_obj, gensym("matrix"));
  inlet_new(&mtx_conv_obj->x_obj, &mtx_conv_obj->x_obj.ob_pd, gensym("matrix"),
            gensym(""));
  mtx_conv_obj->size      = 0;
  mtx_conv_obj->rows      = 0;
                                ...
                                t_atom *argv)
{
  ...
  (void)s; /* unused */
  if (iemmatrix_check(mtx_conv_obj, s, argc, argv, 0))
    return;
...
static void mTXConvMatrix(MTXConv *mtx_conv_obj, t_symbol *s, int argc,
                          t_atom *argv)
{
  ...
  if (!size) {
    ...
    pd_error(mtx_conv_obj,
             "[mtx_conv]: sparse matrix not yet supported: use \"mtx_check\"");
    return;
  } else if (!size_k) {
  ...
  convolveMtx(rows, columns, rows_k, columns_k, mtx_conv_obj->x,
              mtx_conv_obj->k, mtx_conv_obj->y);
  ...
  outlet_anything(mtx_conv_obj->list_outlet, gensym("matrix"), size_y + 2,
                  list_ptr);
}
...
{
  mtx_conv_class =
      class_new(gensym("mtx_conv"), (t_newmethod)newMTXConv,
                (t_method)deleteMTXConv, sizeof(MTXConv), CLASS_DEFAULT, 0);
  class_addbang(mtx_conv_class, (t_method)mTXConvBang);
  class_addmethod(mtx_conv_class, (t_method)mTXConvMatrix, gensym("matrix"),
                  A_GIMME, 0);
  class_addmethod(mtx_conv_class, (t_method)mTXConvKernelMatrix, gensym(""),
                  A_GIMME, 0);
}

void iemtx_conv_setup(void) { mtx_conv_setup(); }
