...
static void proxy_dspstopped(t_proxy *p) { p->p_owner->x_dsp = 0; }

static t_float *get_resized_matrix(const t_float *src, const unsigned int srcR,
                                   const unsigned int srcC,
                                   const unsigned int dstR,
                                   const unsigned int dstC)
{
...
static t_float *resize_and_free(t_float *src, const unsigned int srcR,
                                const unsigned int srcC,
                                const unsigned int dstR,
                                const unsigned int dstC)
{
...
static int matrix_multilde_resize(t_matrix_multilde *x, unsigned int rows,
                                  unsigned int cols)
{
  ...
  if (!x->x_matend || !x->x_matcur || !x->x_inc || !x->x_biginc) {
    pd_error(x, "[%s] failed to resize matrices to [%dx%d]", x->x_name->s_name,
             rows, cols);
    x->x_rows = x->x_cols = 0;
    return 0;
  }
                                       ...
                                       t_atom *argv, int transpose)
{
  ...
  if (argc < 2) {
    pd_error(x, "[%s]: bad matrix: <int:out_rows> <int:in_cols> !",
             x->x_name->s_name);
    return;
  }
  if (iemmatrix_check(x, 0, argc, argv, 0))
    return;
  ...
  if (x->x_outports && x->x_outports != row) {
    pd_error(x,
             "[%s]: cannot change fixed number of output channels (%d) to %d",
...
static void matrix_multilde_matrix(t_matrix_multilde *x, t_symbol *s, int argc,
                                   t_atom *argv)
{
  (void)s; /* unused */
  matrix_multilde_matrix_set(x, argc, argv, 0);
}
static void matrix_multilde_matrixT(t_matrix_multilde *x, t_symbol *s, int argc,
                                    t_atom *argv)
{
...
static void matrix_multilde_element(t_matrix_multilde *x, t_symbol *s, int argc,
                                    t_atom *argv)
{
  ...
  if (argc != 3) {
    pd_error(
        x,
        "[%s]: bad arguments, expected <int:row> <int:column> <float:value>!",
        x->x_name->s_name);
    return;
  }
...
static void matrix_multilde_row(t_matrix_multilde *x, t_symbol *s, int argc,
                                t_atom *argv)
{
...
static void matrix_multilde_col(t_matrix_multilde *x, t_symbol *s, int argc,
                                t_atom *argv)
{
...
static void matrix_multilde_dsp(t_matrix_multilde *x, t_signal **sp)
{
  ...
  if (x->x_setmultiout) {
    ...
    pd_error(
        x,
        "BUG: multichannel enabled but not compile-time multichannel support");
    return;
#endif
  }
  ...
  if (!x->x_outsumbuf) {
    x->x_outsumbufsize = n;
    x->x_outsumbuf =
        (t_sample *)getbytes(x->x_outsumbufsize * sizeof(t_sample));
  } else if (x->x_outsumbufsize != n) {
    x->x_outsumbuf = (t_sample *)resizebytes(
        x->x_outsumbuf, x->x_outsumbufsize * sizeof(t_sample),
        n * sizeof(t_sample));
    x->x_outsumbufsize = n;
  }
...
static void *matrix_multilde_new(t_symbol *s, int argc, t_atom *argv)
{
  ...
  if ((nin < 0) || (nout < 0)) {
    pd_error(0, "[%s] matrix dimensions must not be negative [%dx%d]",
             s->s_name, nout, nin);
    return 0;
  }
  ...
  if (!compat && want_multi) {
    ...
    if (!x->x_setmultiout) {
      ...
      if (warn_multichannel)
        pd_error(x,
                 "[%s] multichannel requested, but iemmatrix was built against "
                 "Pd-%d.%d-%d, which doesn't support it",
                 s->s_name, PD_MAJOR_VERSION, PD_MINOR_VERSION,
                 PD_BUGFIX_VERSION);
      x->x_setmultiout = 0;
    }
    warn_multichannel = 0;
  ...
  x->x_matcur =
      (t_float *)getbytes(x->x_cols * x->x_rows * sizeof(*x->x_matcur));
  x->x_matend =
      (t_float *)getbytes(x->x_cols * x->x_rows * sizeof(*x->x_matend));
  x->x_inc = (t_float *)getbytes(x->x_cols * x->x_rows * sizeof(*x->x_inc));
  x->x_biginc =
      (t_float *)getbytes(x->x_cols * x->x_rows * sizeof(*x->x_biginc));
...
usage:
  pd_error(0,
           "[%s] bad arguments\n\tuse '<int:ouchannels> "
           "<int:inchannels>'\n\tor '-m <int:outchannels> <int:inchannels>'",
           s->s_name);
  return 0;
}
...
{
  class_addmethod(c, (t_method)matrix_multilde_dsp, gensym("dsp"), A_CANT, 0);

  class_addmethod(c, (t_method)matrix_multilde_matrix, gensym("matrix"),
                  A_GIMME, 0);
  class_addmethod(c, (t_method)matrix_multilde_element, gensym("element"),
                  A_GIMME, 0);
  class_addmethod(c, (t_method)matrix_multilde_row, gensym("row"), A_GIMME, 0);
  class_addmethod(c, (t_method)matrix_multilde_col, gensym("col"), A_GIMME, 0);
  class_addmethod(c, (t_method)matrix_multilde_stop, gensym("stop"), 0);
  class_addmethod(c, (t_method)matrix_multilde_time, gensym("time"), A_FLOAT,
                  0);

  /* LATER: can we reuse the 'matrix' method and transpose
   * depending on the compat level? */
  class_addmethod(c, (t_method)matrix_multilde_matrixT, gensym(""), A_GIMME, 0);
...
void mtx_mul_tilde_setup(void)
{
  if (CLASS_MULTICHANNEL && iemmatrix_getpdfun("signal_setmultiout")) {
    ...
    matrix_multilde_mclass =
        class_new(gensym("mtx_mul~"), (t_newmethod)matrix_multilde_new,
                  (t_method)matrix_multilde_free, sizeof(t_matrix_multilde),
                  0 | CLASS_MULTICHANNEL, A_GIMME, 0);
    /* non-multichannel variant */
    /* compatibility with jmz's zexy */
    matrix_multilde_class =
        class_new(gensym("matrix~"), (t_newmethod)matrix_multilde_new,
                  (t_method)matrix_multilde_free, sizeof(t_matrix_multilde), 0,
                  A_GIMME, 0);
  ...
  class_addcreator((t_newmethod)matrix_multilde_new, gensym("mtx_*~"), A_GIMME,
                   0);
  class_addcreator((t_newmethod)matrix_multilde_new, gensym("matrix_mul~"),
                   A_GIMME, 0);

  /* compatibility with tm's iem_matrix */
  class_addcreator((t_newmethod)matrix_multilde_new, gensym("matrix_mul_line~"),
                   A_GIMME, 0);
  ...
  matrix_multilde_proxy =
      class_new(gensym("mtx_*~ proxy"), 0, 0, sizeof(t_proxy), CLASS_PD, 0);
  class_addbang(matrix_multilde_proxy, (t_method)proxy_dspstopped);
}

void iemtx_mul__setup(void) { mtx_mul_tilde_setup(); }
