...
static t_int *mtx_unpack_PerformInactive(t_int *arg) { return (arg + 2); }
...
static t_int *mtx_unpack_PerformActive(t_int *arg)
{
  ...
  const unsigned int maxsamp =
      (x->cols < x->block_size) ? x->cols : x->block_size;
  t_atom *lptr = x->list_in;
  ...
  */
  ...
  if ((num_chan < 1) || (num_chan > MTX_PACK_MAXCHANNELS)) {
     ...
     * and error out otherwise */
      ...
      if (!absname)
        pd_error(x,
                 "[mtx_unpack~] invalid number of channels (%d), default to 1.",
                 num_chan);
    }
    num_chan = 1;
  }
...
usage:
  pd_error(0, "[mtx_unpack~] bad arguments, use '<int:channels>' or '-m "
              "<int:channel>'");
  return 0;
}
...
static void mtx_unpack_matrix(mtx_unpack_tilde *x, t_symbol *s, int argc,
                              t_atom *argv)
{
  int rows, cols;
...
{
  mtx_unpack_tilde_class =
      class_new(gensym("mtx_unpack~"), (t_newmethod)mtx_unpack_new,
                (t_method)mtx_unpack_delete, sizeof(mtx_unpack_tilde),
  ...
  mtx_unpack_tilde_proxy = class_new(gensym("mtx_unpack~ proxy"), 0, 0,
                                     sizeof(t_proxy), CLASS_PD, 0);
  class_addbang(mtx_unpack_tilde_proxy, (t_method)proxy_dspstopped);
}

void iemtx_unpack__setup(void) { mtx_unpack_tilde_setup(); }
