                                 ...
                                 t_atom *argv)
{
  ...
  if (argc > 2) {
    pd_error(x, "mtx_resize : only rows & cols are needed, skipping the rest");
  }
  if (argc == 1) {
                              ...
                              t_atom *argv)
{
  ...
  R   = ROW;
  while (R--) {
    memcpy(x->m.atombuffer + 2 + (ROW - R - 1) * c,
           argv + 2 + (ROW - R - 1) * col, COL * sizeof(t_atom));
  }
...
{
  mtx_resize_class =
      class_new(gensym("mtx_resize"), (t_newmethod)mtx_resize_new, 0,
                sizeof(t_matrixobj), 0, A_GIMME, 0);
  class_addmethod(mtx_resize_class, (t_method)mtx_resize_matrix,
                  gensym("matrix"), A_GIMME, 0);
  class_addmethod(mtx_resize_class, (t_method)mtx_resize_list2, gensym(""),
                  A_GIMME, 0);
}
void iemtx_resize_setup(void) { mtx_resize_setup(); }
