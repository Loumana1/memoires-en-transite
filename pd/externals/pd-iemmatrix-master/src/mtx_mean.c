...
static void mtx_mean_matrix(t_matrixobj *x, t_symbol *s, int argc, t_atom *argv)
{
  if (argc < 2)
    return;
...
{
  mtx_mean_class =
      class_new(gensym("mtx_mean"), (t_newmethod)mtx_mean_new,
                (t_method)matrixobj_free, sizeof(t_matrixobj), 0, 0, 0);
  class_addmethod(mtx_mean_class, (t_method)mtx_mean_matrix, gensym("matrix"),
                  A_GIMME, 0);
}

void iemtx_mean_setup(void) { mtx_mean_setup(); }
