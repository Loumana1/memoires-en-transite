...
typedef int (*t_linalg_SV_decomp)(gsl_matrix *, gsl_matrix *, gsl_vector *,
                                  gsl_vector *);
...
static void deleteMTXSvd(MTXSvd *x) { deleteMTXusvw(x); }
...
{
  if (x->list_u) {
    outlet_anything(x->list_v_out, gensym("matrix"),
                    x->columns * x->columns + 2, x->list_v);
...
static void mTXSvdMatrix(MTXSvd *x, t_symbol *s, int argc, t_atom *argv)
{
...
{
  mtx_svd_class = class_new(gensym("mtx_svd"), (t_newmethod)newMTXSvd,
                            (t_method)deleteMTXSvd, sizeof(MTXSvd),
