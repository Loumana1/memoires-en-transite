...
static void mtx_row_matrix(t_matrixobj *x, t_symbol *s, int argc, t_atom *argv)
{
...
static void mtx_row_list(t_matrixobj *x, t_symbol *s, int argc, t_atom *argv)
{
  ...
  if (argc < x->m.col) {
    pd_error(x, "[mtx_row]: row length is too small for %dx%d-matrix", x->m.row,
             x->m.col);
    return;
  }
  if (x->current_row > x->m.row) {
  ...
  if (x->current_row) {
    ...
    int r = x->m.row;
    while (r--) {
      memcpy(x->m.atombuffer + 2 + r * x->m.col, argv,
             x->m.col * sizeof(t_atom));
    }
  }
  matrixobj_bang(x);
...
{
  mtx_row_class =
      class_new(gensym("mtx_row"), (t_newmethod)mtx_row_new,
  ...
  class_addmethod(mtx_row_class, (t_method)mtx_row_matrix, gensym("matrix"),
                  A_GIMME, 0);
  class_addmethod(mtx_row_class, (t_method)mtx_row_float, gensym(""), A_FLOAT,
                  0);
}
