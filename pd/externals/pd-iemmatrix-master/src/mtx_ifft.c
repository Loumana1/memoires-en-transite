...
static void resizeFFT(MtxIFFT *x, int rows, int columns)
{
  ...
  if (columns != x->columns) {
    if (x->plan)
      iemfft_destroy_plan(x->plan);
    x->plan = iemfft_plan_ifft_1d(columns, x->c_in, x->c_out, DEFAULT);
  }
...
static void mtxIFFTMatrixCold(MtxIFFT *x, t_symbol *s, int argc, t_atom *argv)
{
  (void)s; /* unused */
...
static void mtxIFFTMatrixHot(MtxIFFT *x, t_symbol *s, int argc, t_atom *argv)
{
  ...
  if (msg_size != x->msg_size) {
    pd_error(x,
             "[mtx_ifft]: left matrix has other dimensions than right matrix");
    return;
  } else if (columns < 4) {
    pd_error(x, "[mtx_ifft]: matrix must have at least 4 columns");
    return;
  ...
  outlet_anything(x->msg_im_out, gensym("matrix"), x->msg_size + 2, msg_im);
  outlet_anything(x->msg_re_out, gensym("matrix"), x->msg_size + 2, msg_re);
}

void mtx_ifft_setup(void)
{
  mtx_ifft_class =
      class_new(gensym("mtx_ifft"), (t_newmethod)newMtxIFFT,
                (t_method)deleteMtxIFFT, sizeof(MtxIFFT), CLASS_DEFAULT, 0);
  class_addbang(mtx_ifft_class, (t_method)mtxIFFTBang);
  class_addmethod(mtx_ifft_class, (t_method)mtxIFFTMatrixHot, gensym("matrix"),
                  A_GIMME, 0);
  class_addmethod(mtx_ifft_class, (t_method)mtxIFFTMatrixCold, gensym(""),
                  A_GIMME, 0);

  iemfft_init(mtx_ifft_class);
}

void iemtx_ifft_setup(void) { mtx_ifft_setup(); }
