...
static void deleteMTXFind(MTXfind *mtx_find_obj)
{
  if (mtx_find_obj->list_out) {
    freebytes(mtx_find_obj->list_out,
...
static void *newMTXFind(t_symbol *s, int argc, t_atom *argv)
{
    ...
    if (argv[0].a_type == A_SYMBOL) {
      ...
      if (argc >= 2) {
        if (argv[1].a_type != A_SYMBOL) {
          ...
          pd_error(mtx_find_obj, "[%s]: 2nd arg ignored. supposed to be float",
                   s->s_name);
        }
      }
    } else {
      ...
      if (argc >= 2) {
        if (argv[1].a_type == A_SYMBOL) {
          ...
          pd_error(mtx_find_obj,
                   "[%s]: 2nd arg ignored. supposed to be symbolic, e.g. "
                   "\"row\", \"col\", \":\", \"mtx\"",
                   s->s_name);
        }
      }
  ...
  mtx_find_obj->list_outlet =
      outlet_new(&mtx_find_obj->x_obj, gensym("matrix"));

  pd_error(
      mtx_find_obj,
      "[mtx_find]: this object is likely to change! not really for use yet");
...
static void findFirstNonZeroRow(const int rows, const int columns, t_atom *x,
                                t_atom *y)
{
  int       offset;
  int       pos;
...
static void findLastNonZeroRow(const int rows, const int columns, t_atom *x,
                               t_atom *y)
{
  int       offset;
  int       pos;
...
static void findFirstNonZeroColumn(const int rows, const int columns, t_atom *x,
                                   t_atom *y)
{
  int       offset;
  int       pos;
...
static void findLastNonZeroColumn(const int rows, const int columns, t_atom *x,
                                  t_atom *y)
{
  int       offset;
  int       pos;
...
static void mTXFindMatrix(MTXfind *mtx_find_obj, t_symbol *s, int argc,
                          t_atom *argv)
{
  ...
  /* size check */
  if (iemmatrix_check(mtx_find_obj, s, argc, argv, 0))
    return;
  ...
  if (size != mtx_find_obj->size) {
    if (!list_out) {
      ...
      list_out = (t_atom *)resizebytes(
          list_out, sizeof(t_atom) * (mtx_find_obj->size + 2),
...
{
  mtx_find_class = class_new(gensym("mtx_find"), (t_newmethod)newMTXFind,
                             (t_method)deleteMTXFind, sizeof(MTXfind),
                             CLASS_DEFAULT, A_GIMME, 0);
  class_addbang(mtx_find_class, (t_method)mTXFindBang);
  class_addmethod(mtx_find_class, (t_method)mTXFindMatrix, gensym("matrix"),
                  A_GIMME, 0);
...
void iemtx_find_setup(void) { mtx_find_setup(); }
