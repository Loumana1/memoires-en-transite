...
typedef fftwf_plan (*t_fftw32_plan_dft_1d)(int, t_complexfloat *,
                                           t_complexfloat *, signed, unsigned);
typedef fftwf_plan (*t_fftw32_plan_dft_c2r_1d)(int, t_complexfloat *, float *,
                                               unsigned);
typedef fftwf_plan (*t_fftw32_plan_dft_r2c_1d)(int, float *, t_complexfloat *,
                                               unsigned);
...
typedef fftw_plan (*t_fftw64_plan_dft_1d)(int, t_complexdouble *,
                                          t_complexdouble *, signed, unsigned);
typedef fftw_plan (*t_fftw64_plan_dft_c2r_1d)(int, t_complexdouble *, double *,
                                              unsigned);
typedef fftw_plan (*t_fftw64_plan_dft_r2c_1d)(int, double *, t_complexdouble *,
                                              unsigned);
...
static void mayerfloat2complex(const unsigned int N, const t_float *in,
                               t_complex *out)
{
...
static void complex2mayerfloat(const unsigned int N, const t_complex *in,
                               t_float *out)
{
...
static void complex_deinterleave(unsigned int N, const t_complex *in,
                                 t_float *re, t_float *im)
{
...
static void complex_interleave(unsigned int N, const t_float *re, t_float *im,
                               t_complex *out)
{
...
struct iemfft_plan {
  ...
  void *
      _in; /* in case the input data must not be modified; or we need to type-convert */
  void *_out; /* for type-converting the output data */

  /* these are needed for conversions */
  void *inptr,
      *outptr; /* just pointers to the actually used input/output pointers */
  size_t insize,
      outsize; /* the actual number of elements in the input/output poitners */
...
t_iemfft_plan iemfft_plan_rifft_1d(int n0, t_complex *in, t_float *out,
                                   t_iemfft_flag flags)
{
...
t_iemfft_plan iemfft_plan_rfft_1d(int n0, t_float *in, t_complex *out,
                                  t_iemfft_flag flags)
{
  t_iemfft_plan       plan      = calloc(1, sizeof(*plan));
  const t_iemfft_flag precision = flags2precision(flags);
...
static t_iemfft_plan _fft_plan_1d(int n0, t_complex *in, t_complex *out,
                                  int inverse, t_iemfft_flag flags)
{
  t_iemfft_plan       plan      = calloc(1, sizeof(*plan));
  const t_iemfft_flag precision = flags2precision(flags);
  plan->flags                   = flags;
  ...
  case PREFER_SINGLE:
    ...
    _out = plan->_out = iemfft_malloc(n0 * sizeof(*_out));
    plan->fftw32_plan =
        _fftw32_plan_dft_1d(n0, _in, _out, inverse ? +1 : -1, FFTW_ESTIMATE);
#else
    plan->fftw32_plan =
        _fftw32_plan_dft_1d(n0, in, out, inverse ? +1 : -1, FFTW_ESTIMATE);
#endif
    break;
  case PREFER_DOUBLE:
#if PD_FLOATSIZE != 64
    ...
    _out = plan->_out = iemfft_malloc(n0 * sizeof(*_out));
    plan->fftw64_plan =
        _fftw64_plan_dft_1d(n0, _in, _out, inverse ? +1 : -1, FFTW_ESTIMATE);
#else
    plan->fftw64_plan =
        _fftw64_plan_dft_1d(n0, in, out, inverse ? +1 : -1, FFTW_ESTIMATE);
#endif
    break;
  default:
...
t_iemfft_plan iemfft_plan_fft_1d(int n0, t_complex *in, t_complex *out,
                                 t_iemfft_flag flags)
{
  return _fft_plan_1d(n0, in, out, 0, flags);
}

t_iemfft_plan iemfft_plan_ifft_1d(int n0, t_complex *in, t_complex *out,
                                  t_iemfft_flag flags)
{
...
#define register_fftw64(name)                                                  \
  _fftw64_##name = iemmatrix_get_stub(stringify(fftw_##name), c)
    ...
    have_fftw32 =
        (1 && _fftw32_malloc && _fftw32_free && _fftw32_destroy_plan &&
         _fftw32_execute && _fftw32_plan_dft_1d && _fftw32_plan_dft_c2r_1d &&
    ...
    have_fftw64 =
        (1 && _fftw64_malloc && _fftw64_free && _fftw64_destroy_plan &&
         _fftw64_execute && _fftw64_plan_dft_1d && _fftw64_plan_dft_c2r_1d &&
