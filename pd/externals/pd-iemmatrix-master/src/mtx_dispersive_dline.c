                                            ...
                                            t_floatarg              f)
{
  if ((f < 1.0f) && (f > -1.0f)) {
    ...
    pd_error(x, "[mtx_dispersive_dline]: stable allpass coefficient must be "
                "-1<lambda<1");
  }
}
                                        ...
                                        int argc, t_atom *argv)
{
  ...
  if (argc > 1) {
    ...
    if ((channels < 1) || (channels > 1000)) {
      pd_error(x, "[mtx_dispersive_dline]: number of channels (input rows) "
                  "must lie between 1 and 1000!");
      return;
    }
  }
                                        ...
                                        int argc, t_atom *argv)
{
  ...
  for (c = 0, n2 = 0; c < x->size; c += x->length) {
    for (n = 0; n < samples; n++, n2++) {
      allpass_chain_cycle(atom_getfloat(argv + n2), x->tap + c, x->z + c,
                          x->length, x->lambda);
    }
  }
...
{
  t_mtx_dispersive_dline *x =
      (t_mtx_dispersive_dline *)pd_new(mtx_dispersive_dline_class);
  t_float length = 1;
  t_float lambda = 0;
  t_atom  resize_msg[2];
  (void)s; /* unused */
  ...
  case 2:
    ...
    /* fall through */
  case 1: length = atom_getfloat(argv);
  }
...
void mtx_dispersive_dline_setup(void)
{
  mtx_dispersive_dline_class = class_new(
      gensym("mtx_dispersive_dline"), (t_newmethod)mtx_dispersive_dline_new,
      (t_method)mtx_dispersive_dline_delete, sizeof(t_mtx_dispersive_dline),
      CLASS_DEFAULT, A_GIMME, 0);
  class_addmethod(mtx_dispersive_dline_class,
                  (t_method)mtx_dispersive_dline_matrix, gensym("matrix"),
                  A_GIMME, 0);
  class_addmethod(mtx_dispersive_dline_class,
                  (t_method)mtx_dispersive_dline_reset, gensym("reset"), 0);
  class_addmethod(mtx_dispersive_dline_class,
                  (t_method)mtx_dispersive_dline_resize, gensym("resize"),
                  A_GIMME, 0);
  class_addmethod(mtx_dispersive_dline_class,
                  (t_method)mtx_dispersive_dline_set_lambda, gensym("lambda"),
                  A_DEFFLOAT, 0);
...
void iemtx_dispersive_dline_setup(void) { mtx_dispersive_dline_setup(); }
