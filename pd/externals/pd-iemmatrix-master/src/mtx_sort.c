...
static void *newMTXSort(t_symbol *s, int argc, t_atom *argv)
{
    ...
    if (argv[0].a_type == A_SYMBOL) {
      ...
      if (argc >= 2) {
        if (argv[1].a_type != A_SYMBOL) {
          ...
          pd_error(mtx_sort_obj,
                   "[mtx_sort]: 2nd arg ignored. supposed to be float");
        }
      }
    } else {
      ...
      if (argc >= 2) {
        if (argv[1].a_type == A_SYMBOL) {
          ...
          pd_error(mtx_sort_obj, "[mtx_sort]: 2nd arg ignored. supposed to be "
  ...
  mtx_sort_obj->list_outlet1 =
      outlet_new(&mtx_sort_obj->x_obj, gensym("matrix"));
  mtx_sort_obj->list_outlet2 =
      outlet_new(&mtx_sort_obj->x_obj, gensym("matrix"));
...
static void mTXSortMatrix(MTXSort *mtx_sort_obj, t_symbol *s, int argc,
                          t_atom *argv)
{
  ...
  /* size check */
  if (iemmatrix_check(mtx_sort_obj, s, argc, argv, 0))
    return;
  ...
  if ((!x) || (!list_out1) || (!list_out2) /*||(!y)*/) {
    ...
    x = (t_float *)resizebytes(x, sizeof(t_float) * (mtx_sort_obj->size),
                               sizeof(t_float) * (size));
    i = (t_float *)resizebytes(i, sizeof(t_float) * (mtx_sort_obj->size),
                               sizeof(t_float) * (size));
...
{
  mtx_sort_class = class_new(gensym("mtx_sort"), (t_newmethod)newMTXSort,
                             (t_method)deleteMTXSort, sizeof(MTXSort),
                             CLASS_DEFAULT, A_GIMME, 0);
  class_addbang(mtx_sort_class, (t_method)mTXSortBang);
  class_addmethod(mtx_sort_class, (t_method)mTXSortMatrix, gensym("matrix"),
                  A_GIMME, 0);
...
void iemtx_sort_setup(void) { mtx_sort_setup(); }
