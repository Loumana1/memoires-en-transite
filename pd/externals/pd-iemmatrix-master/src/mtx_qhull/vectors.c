...
points_t initPoints(const float *x, const float *y, const float *z,
...
void appendPoints(points_t *points, const float *x, const float *y,
...
{
  return lengthVector(
      crossProduct(line.direction, subtractVectors(point, line.point)));
}

float distancePointLineOnPlane(vector_t const point, const line_t line,
                               const plane_t plane)
{
  vector_t normal_in_plane =
      normalizeVector(crossProduct(line.direction, plane.normal));
...
plane_t planeFromThreePoints(const vector_t a, const vector_t b,
                             const vector_t c)
{
...
size_t getNumPoints(const points_t points) { return points.num_points; }
