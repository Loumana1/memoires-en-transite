...
void reallocateList(list_t *list, const size_t length)
{
  if (length > 0) {
    if (getLength(*list) == 0) {
      *list = allocateList(length);
      ...
      if (list->length != length) {
        list->entries =
            (entry_t *)realloc(list->entries, length * sizeof(entry_t));
      }
      if (list->entries != 0) {
        list->length = length;
      } else {
...
size_t getLength(const list_t list) { return list.length; }
...
list_t getSubList(const list_t list, const list_t indices)
{
  ...
  for (i = 0; i < getLength(new_list); i++) {
    ...
    setEntry(new_list, i, getEntry(list, entry_getIndex(&e1)));
  }
  return new_list;
}
                        ...
                        const index_t stop)
{
  ...
  index_t i, j;
  int     incr;
  if ((start > 0) && (stop > 0) && (start < getLength(list)) &&
      (stop < getLength(list))) {
    if (start > stop) {
      incr     = -1;
