...
static facet_t *getFacetByIndex(const list_t facets, const index_t index)
{
  entry_t e = getEntry(facets, index);
  return ((facet_t *)entry_getPointer(&e));
}

index_t getTriangleCorner(const zhull_t *const zh, const index_t triangle_idx,
...
static void getHorizonEdgeByIndex(index_t *corners, const list_t horizon_fcts,
                                  const list_t  horizon_fcts_edges,
                                  const list_t  other_horizon_edges,
                                  const index_t index)
{
  index_t i =
      (index + getLength(horizon_fcts_edges)) % getLength(horizon_fcts_edges);
...
static void removeFacetByPointer(zhull_t *const zh, facet_t *const pointer)
{
...
static void removeFacetByPointerList(zhull_t *const zh, const list_t pointers)
{
  index_t i;
  for (i = 0; i < getLength(pointers); i++) {
    removeFacetByPointer(zh, getFacetByIndex(pointers, i));
  }
}

static void removeFacetByIndex(zhull_t *const zh, const index_t index)
{
  removeFacetByPointer(zh, getFacetByIndex(zh->facets, index));
}

static void removeFacetByIndexList(zhull_t *const zh, const list_t indices)
{
  facet_t *f;
  index_t  i;
...
static line_t getLine(const zhull_t *const zh, const facet_t *const f,
...
zhull_t zhullInitPoints(const float *x, const float *y, const float *z,
...
static void dividePointsBetweenNewFacets(zhull_t *const zh, const list_t assoc,
                                         const list_t new_facets)
{
  ...
  for (i = 0; i < getLength(assoc); i++) {
    ...
    for (j = 0; j < getLength(new_facets); j++) {
      ...
      d = distancePointPlane(getPoint(zh->pts, idx), f->plane);
    ...
    if (d >= TOL_OUTSIDEPOINT) {
      ...
      if (getLength(point_inside_facet_list) > 0) {
        ...
        appendListToList(&(zh->facets_with_insidepoints),
                         point_inside_facet_list);
  ...
  if (getNumPoints(zh->pts) >= 3) {
    ...
    if (getLength(new_facets) == 2) {
      do { // circumvent coincident points
        if (distancePointPoint(getPoint(zh->pts, idx[0]),
                               getPoint(zh->pts, idx[1])) > TOL_DEGENERATE) {
          break;
        } else {
          idx[1]++;
        }
      ...
      if (idx[1] < getNumPoints(zh->pts)) {
        do { // circumvent degenerate triangles
          ...
          if (lengthVector(normalOfListedPoints(zh->pts, list)) >
              TOL_DEGENERATE) {
            break;
          } else {
            idx[2]++;
        ...
        if (idx[2] < getNumPoints(zh->pts)) {
          ...
          getFacetByIndex(new_facets, 0)->neighbors = initConstantList(
              entry_makePointer(getFacetByIndex(new_facets, 1)), 3);
          getFacetByIndex(new_facets, 1)->neighbors = initConstantList(
              entry_makePointer(getFacetByIndex(new_facets, 0)), 3);
          for (i = 0; i < 2; i++) {
            getFacetByIndex(new_facets, i)->plane = planeFromListedPoints(
                zh->pts, getFacetByIndex(new_facets, i)->corners);
...
static void printHorizonEdges(list_t *horizon_fcts, list_t *horizon_fcts_edges,
...
static void sortHorizonEdges(list_t *horizon_fcts, list_t *horizon_fcts_edges,
                             list_t *other_horizon_edges)
{
  index_t i, j;
  entry_t ei;
  ...
  for (i = 0; i < getLength(*horizon_fcts_edges) - 1; i++) {
    ...
    for (j = i + 1; j < getLength(*horizon_fcts_edges); j++) {
      ...
      if ((c1[1] == c2[0]) && (c1[0] != c2[1])) {
        ...
        setEntry(*other_horizon_edges, j,
                 getEntry(*other_horizon_edges, i + 1));
    ...
    if (j == getLength(*horizon_fcts_edges)) {
      ...
      for (j = i + 1; j < getLength(*horizon_fcts_edges); j++) {
        ...
        if (c1[1] == c2[0]) {
          ...
          setEntry(*horizon_fcts, i + 1, ei);
          ei = getEntry(*horizon_fcts_edges, j);
          setEntry(*horizon_fcts_edges, j,
                   getEntry(*horizon_fcts_edges, i + 1));
          setEntry(*horizon_fcts_edges, i + 1, ei);
          ei = getEntry(*other_horizon_edges, j);
          setEntry(*other_horizon_edges, j,
                   getEntry(*other_horizon_edges, i + 1));
      ...
      for (j = 0; j < getLength(f->neighbors); j++) {
        ...
        if (d >= TOL_OUTSIDEPOINT) { // visit visible neighbors
          ...
          appendToList(other_horizon_edges,
                       entry_makeIndex(getNumPoints(zh->pts)));
        }
      }
      ...
      for (j = 0; j < getLength(f->neighbors); j++) {
        ...
        if (d >= TOL_INSIDEPOINT) { // coincident facet
          ...
          if ((innerProduct(f->plane.normal, n->plane.normal) <
            ...
            appendToList(other_horizon_edges, getEntry(f->corners, j));
...
static void initNewFacets(zhull_t *zh, index_t point_index, list_t new_facets,
                          ...
                          list_t other_horizon_edges)
{
  ...
  for (i = 0; i < getLength(new_facets); i++) {
    ...
    if (entry_getPointer(&e) != 0) {
      ...
      setEntry(getFacetByIndex(horizon_fcts, i)->neighbors, entry_getIndex(&e),
      ...
      setEntry(neighbors, 0, getEntry(new_facets, i));
    }

    getFacetByIndex(new_facets, i)->neighbors = initList(entry_array, 3);
...
int calculateZHull(zhull_t *zh)
{
  ...
  if (getNumPoints(zh->pts) != 0) {
    ...
    while (((getLength(zh->facets_with_insidepoints) > 0) ||
      ...
      //            printf("point %d\n",pi);
      removeVisibleFacetsGetHorizonAndAvailablePoints(
          zh, pi, f, &horizon_fcts, &horizon_fcts_edges, &other_horizon_edges,
          &available_points);
...
void printZhull(const zhull_t *const zh)
{
  ...
  for (fi = 0; fi < getLength(zh->facets); fi++) {
    printf("facet %lu<%p>: ", (unsigned long)fi,
           getFacetByIndex(zh->facets, fi));
