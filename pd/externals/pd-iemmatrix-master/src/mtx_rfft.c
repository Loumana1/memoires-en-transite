...
static void complex2list(int n, t_complex *c, t_atom *re, t_atom *im)
{
  while (n--) {
    SETFLOAT(re, c->re);
    SETFLOAT(im, c->im);
...
static void mTXRfftMatrix(MTXRfft *x, t_symbol *s, int argc, t_atom *argv)
{
  int rows    = atom_getint(argv++);
  int columns = atom_getint(argv++);
  int columns_re =
      (columns >> 1) + 1; /* N/2+1 samples needed for real part of realfft */
  ...
  if (!size) {
    ...
    pd_error(x,
             "[mtx_rfft]: sparse matrix not yet supported: use \"mtx_check\"");
    return;
  } else if (columns < 4) {
  ...
  outlet_anything(x->list_im_out, gensym("matrix"), x->list_size, list_im);
  outlet_anything(x->list_re_out, gensym("matrix"), x->list_size, list_re);
}

void mtx_rfft_setup(void)
{
  mtx_rfft_class = class_new(gensym("mtx_rfft"), (t_newmethod)newMTXRfft,
                             (t_method)deleteMTXRfft, sizeof(MTXRfft),
                             CLASS_DEFAULT, A_GIMME, 0);
  class_addbang(mtx_rfft_class, (t_method)mTXRfftBang);
  class_addmethod(mtx_rfft_class, (t_method)mTXRfftMatrix, gensym("matrix"),
                  A_GIMME, 0);

  iemfft_init(mtx_rfft_class);
}

void iemtx_rfft_setup(void) { mtx_rfft_setup(); }
