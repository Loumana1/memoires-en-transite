                                 ...
                                 t_symbol *type)
{
  ...
  fd = canvas_open(x->x_canvas, s->s_name, "", filenamebuf, &filenamebufptr,
                   MAXPDSTRING, 1);
  if (fd < 0) {
    pd_error(x, "[mtx_sndfileread]: failed to open %s : %s", s->s_name,
             filenamebuf);
    return;
  }
  myclose(fd);
  ...
  if (!(x->x_sndfileread = my_open(filenamebuf, SFM_READ, &x->x_sfinfo))) {
    pd_error(x, "[mtx_sndfileread]: failed to sfopen %s : %s", s->s_name,
             filenamebuf);
    mtx_sndfileread_close(x);
    return;
  }
...
static void mtx_sndfileread_frame(t_mtx_sndfileread *x)
{
  ...
  if ((!x->x_sndfileread) || (x->num_chan <= 0)) {
    pd_error(
        x,
        "[mtx_sndfileread] read_frame: no or damaged file opened for reading");
    return;
  }

  if (!(x->x_float) || (x->num_frames < 1)) {
    if (!(x->x_outlist =
              (t_atom *)getbytes(sizeof(t_atom) * (2 + x->num_chan)))) {
...
static void mtx_sndfileread_frames(t_mtx_sndfileread *x, t_float f)
{
  ...
  if ((!x->x_sndfileread) || (x->num_chan <= 0)) {
    pd_error(
        x,
        "[mtx_sndfileread] read_frames: no or damaged file opened for reading");
    return;
  }

  if (!(x->x_float) || (x->num_frames < num_frames)) {
    if (!(x->x_outlist = (t_atom *)getbytes(sizeof(t_atom) *
                                            (2 + num_frames * x->num_chan)))) {
      pd_error(x, "[mtx_sndfileread] read_frames: out of memory!");
      return;
    }
    if (!(x->x_float =
              (float *)getbytes(sizeof(float) * num_frames * x->num_chan))) {
  ...
  if ((frames_read = my_readf_float(x->x_sndfileread, x->x_float,
...
{
  mtx_sndfileread_class = class_new(
      gensym("mtx_sndfileread"), (t_newmethod)mtx_sndfileread_new,
