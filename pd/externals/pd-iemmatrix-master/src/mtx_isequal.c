...
static int _isequal_list2float(int argc, const t_atom *argv, const t_float ref,
                               const t_float epsilon)
{
  if (0. == epsilon) {
    while (argc--) {
...
static int _isequal_list2list(int argc, const t_atom *argv, const t_atom *ref,
                              const t_float epsilon)
{
  if (0. == epsilon) {
    while (argc--) {
...
{
  (void)s; /* unused */
  if (iemmatrix_check(x, s, argc, argv, IEMMATRIX_CHECK_CRIPPLED))
    return;
                               ...
                               t_atom *argv)
{
  ...
  int result = _isequal_list2list(argc - 2, argv + 2, x->x.m2.atombuffer + 2,
                                  x->epsilon);
  outlet_float(x->x.x_obj.ob_outlet, (t_float)result);
}
...
{
  if (argc) {
    t_mtx_isequalscalar *x =
        (t_mtx_isequalscalar *)pd_new(mtx_isequalscalar_class);
    if (argc > 1) {
...
{
  mtx_isequal_class = class_new(
      gensym("mtx_isequal"), (t_newmethod)mtx_isequal_new,
      (t_method)mtx_binmtx_free, sizeof(t_mtx_isequalmtx), 0, A_GIMME, 0);
  ...
  mtx_isequalscalar_class =
      class_new(gensym("mtx_isequal"), 0, (t_method)mtx_binscalar_free,
                sizeof(t_mtx_isequalscalar), 0, 0);
  class_addmethod(mtx_isequalscalar_class, (t_method)mtx_isequalscalar_matrix,
                  gensym("matrix"), A_GIMME, 0);
  class_addlist(mtx_isequalscalar_class, mtx_isequalscalar_list);
  class_addbang(mtx_isequalscalar_class, mtx_binscalar_bang);
  class_addmethod(mtx_isequalscalar_class, (t_method)mtx_isequalscalar_eps,
                  gensym("eps"), A_FLOAT, 0);
}
