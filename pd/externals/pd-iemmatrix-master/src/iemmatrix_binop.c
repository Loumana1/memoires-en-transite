...
static void mtx_binop_sendmatrix(t_matrix_binop *x)
{
  ...
  outlet_anything(x->x_obj.ob_outlet, gensym("matrix"), argc + 2,
                  x->m.atombuffer);
}
 ...
 */
static void mtx_binop_m2f(t_matrix_binop *x, t_matrix *m1, t_float f2,
                          int reverse)
{
...
static void mtx_binop_matrix(t_matrix_binop *x, t_symbol *s, int argc,
                             t_atom *argv)
{
...
static void mtx_binop_matrix2(t_matrix_binop *x, t_symbol *s, int argc,
                              t_atom *argv)
{
  if (iemmatrix_check(x, s, argc, argv, 0))
    return;
...
static void mtx_binopscalar_bang(t_matrix_binop *x)
{
  ...
  if (x->x_selector == gensym("matrix"))
    ...
    outlet_anything(x->x_obj.ob_outlet, x->x_selector, x->x_argc,
                    x->m.atombuffer + 2);
  }
}

static void mtx_binopscalar_matrix(t_matrix_binop *x, t_symbol *s, int argc,
                                   t_atom *argv)
{
  if (iemmatrix_check(x, s, argc, argv, 0))
    return;
...
static void *mtx_binop_new(t_symbol *s, int argc, t_atom *argv)
{
  ...
  if (argc > 1) {
    pd_error(x, "%s : extra arguments ignored",
             (s && s->s_name) ? s->s_name : "matrix object");
  }
  return (x);
}
...
void iemmatrix_binop_setup(const char *classname, const char *helpname,
                           iemmatrix_binopfun_t *fun, ...)
{
  if (!classname || !fun)
    return;
  ...
  cls = class_new(s, (t_newmethod)mtx_binop_new, (t_method)mtx_binop_free,
                  sizeof(t_matrix_binop), 0, A_GIMME, A_NULL);
  class_addmethod(cls, (t_method)mtx_binop_matrix, gensym("matrix"), A_GIMME,
                  A_NULL);
  class_addmethod(cls, (t_method)mtx_binop_matrix2, gensym(""), A_GIMME,
                  A_NULL);
  ...
  scalarcls =
      class_new(s, 0, /* no constructor */
                (t_method)mtx_binop_free, sizeof(t_matrix_binop), 0, A_NULL);
  class_addmethod(scalarcls, (t_method)mtx_binopscalar_matrix, gensym("matrix"),
                  A_GIMME, A_NULL);
