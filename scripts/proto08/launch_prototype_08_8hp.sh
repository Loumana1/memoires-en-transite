#!/bin/bash
# Lance le prototype 08 — variante 8HP (SONS_V3, 12 voix + 2 nappes)
# Mémoires en transit / Micro-opacités
set -euo pipefail

MET_ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
PATCH="$MET_ROOT/pd/prototype_08_fsm_8hp.pd"

if [[ ! -f "$PATCH" ]]; then
  echo "Patch introuvable. Lancer: python3 scripts/gen_prototype_08_8hp.py"
  exit 1
fi
