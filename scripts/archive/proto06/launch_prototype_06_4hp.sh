#!/bin/bash
# Lance le prototype 06 — variante 4HP (sorties 1-2-3-4) — Mémoires en transit
set -euo pipefail

MET_ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
PATCH="$MET_ROOT/pd/prototype_06_fsm_4hp.pd"

if [[ ! -f "$PATCH" ]]; then
  echo "Patch introuvable. Lancer: python3 scripts/gen_prototype_06_4hp.py"
  exit 1
fi
