#!/bin/bash
# Lance le prototype 06 — variante 6HP (sorties 1-2-4-5-6, sortie 3 inutilisée) — Mémoires en transit
set -euo pipefail

MET_ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
PATCH="$MET_ROOT/pd/prototype_06_fsm_6hp.pd"

if [[ ! -f "$PATCH" ]]; then
  echo "Patch introuvable. Lancer: python3 scripts/gen_prototype_06_6hp.py"
  exit 1
fi
