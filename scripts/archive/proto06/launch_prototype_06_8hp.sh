#!/bin/bash
# Lance le prototype 06 — variante 8HP (sorties 1..8, VISU samples + historique)
# Cycle 1 artiste: CORTEX (opaque) → HIPPOCAMPE → RECONSTRUCTION
# Mémoires en transit
set -euo pipefail

MET_ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
PATCH="$MET_ROOT/pd/prototype_06_fsm_8hp.pd"

if [[ ! -f "$PATCH" ]]; then
  echo "Patch introuvable. Lancer: python3 scripts/gen_prototype_06_8hp.py"
  exit 1
fi
